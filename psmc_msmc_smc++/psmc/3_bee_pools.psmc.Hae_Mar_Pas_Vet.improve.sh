...
# AndHae_hifi_asm.aligned.bam.fq.split_100000.psmcfa
...
for p in "4+25*2+4+6" "5+30*2+5+10" "6+20*2+6+8"; do
    psmc -N25 -t15 -r5 -b -p $p -o output_$p.psmc input.psmcfa
done

# Final Tips

✔️ Start with 4+25*2+4+6 (default).
✔️ Adjust t1 for more recent resolution.
✔️ Change n1*t2 for smoother intermediate estimates.
✔️ Increase t3 for deeper time coverage.
...
# psmc_plot.pl -S -R -u 3.6e-09 -g 1 ../pas_REF_pas_DP200_1500x.psmc.plot ../pas_REF_pas_DP200_1500x.combined.psmc

# epstopdf ($Id: epstopdf.pl 66407 2023-03-06 23:44:49Z karl $) 2.31
# !!! Error: Cannot open Ghostscript for piped input: gs -q -dSAFER -dNOPAUSE -dBATCH -dCompatibilityLevel=1.5 -sDEVICE=pdfwrite -sOutputFile=round-1.AndHae_hifi_asm.aligned.bam.fq.split_100000.psmc.pdf -dPDFSETTINGS#/prepress -dMaxSubsetPct=100 -dSubsetFonts=true -dEmbedAllFonts=true -dAutoRotatePages#/None - -c quit
