...
result_path="/home/yzliu/eDNA/faststorage/yzliu/DK_proj/data/bee_proj_data/psmc_output/bootstrap"
setwd(result_path)
...
file_list_a <- c(             
                ...
                #5X
                "AndHae_New_REF_AndHae.with_singleton_sfs_390_1500x_sm_genic",
                "AndMar_New_REF_AndMar.with_singleton_sfs_400_1500x_sm_genic",
                "BomPas_New_REF_BomPas.with_singleton_sfs_340_1500x_sm_genic",
                "BomVet_New_REF_BomVet.with_singleton_sfs_290_1500x_sm_genic"
...
file_path <- fs::dir_ls(path=paste0(result_path), recurse = 2, fail=TRUE, type = "file", glob = "*combined.psmc.0.txt")
file_path_hae <- file_path[1]
data_hae <- read.table(file_path_hae,header=FALSE,sep="\t")
#head(data_hae)
# check the number of rows
str(data_hae)
Species <- rep("A.haemorrhoa",50)
...
file_path_mar <- file_path[2]
data_mar <- read.table(file_path_mar,header=FALSE,sep="\t")
#head(data_hae)
# check the number of rows
str(data_mar)
Species <- rep("A.marginata",50)
data_mar_new <- cbind(Species,data_mar)
head(data_mar_new)

file_path_pas <- file_path[3]
data_pas <- read.table(file_path_pas,header=FALSE,sep="\t")
#head(data_hae)
# check the number of rows
str(data_pas)
Species <- rep("B.pascuorum",50)
data_pas_new <- cbind(Species,data_pas)
head(data_pas_new)

file_path_vet <- file_path[4]
data_vet <- read.table(file_path_vet,header=FALSE,sep="\t")
#head(data_hae)
# check the number of rows
str(data_vet)
Species <- rep("B.veteranus",50)
...
merged_data_new <- merged_data %>%
  mutate(across(c(V1, V2, V3, V4, V5), as.numeric))
tail(merged_data_new)
dim(merged_data_new)
str(merged_data_new)
...
plot <- ggplot(merged_data_new, aes(x = year, y = Ne_median, color = species, fill = species)) +
  ...
  labs(title = "Median Ne Over Years by Species (5x)",
       x = "Year",
       y = "Ne Median",
       color = "Species",
       fill = "Species") +
...
# psmc
plot <- ggplot(merged_data_new, aes(x = V1/1000, y = V2, color = Species, fill = Species)) +
  geom_line(linewidth = 1) +
  #geom_ribbon(aes(ymin = Ne_12.5., ymax = Ne_87.5.), alpha = 0.2, color = NA) +
  labs(title = "PSMC based on HiFi reads",
       x = "Years ago (x1000)",
       y = "Effective population size (x10000)",
       color = "Species",
       fill = "Species") +
  theme_minimal() +
  theme(
...
#install.packages("scales") for function "label_number"
library(scales)
install.packages("Cairo") # had issue
library("Cairo")
# check where R is installed
r$> .libPaths()
[1] "/home/yzliu/miniforge3/envs/R/lib/R/library"
conda activate R
...
# Import fonts (only need to do this once)
font_import()
loadfonts(device = "pdf")


## final
p1_initial <- ggplot(merged_data_new, aes(x = V1/1000, y = V2, color = Species, fill = Species)) +
  #geom_ribbon(aes(ymin = Ne_12.5., ymax = Ne_87.5., fill = species), size = 0.1, alpha = 0.1) +  # Ribbon for the range
  geom_step(size = 1, alpha = 0.7) +  # Line for Ne_median
    ## add legend manually
  ...
  labs(title = "PSMC based on HiFi reads",
       y = expression(paste("Effective population size (x10"^4,")")),
       x = expression(paste("Years ago (x10"^3,", g=1, ", mu == 3.6,"x", 10^{-9}, ")"))) +  # Correct label for the x-axis
  #theme_minimal() +
  theme_classic() +
  #guides(color = guide_legend(title = NULL,order = 1, override.aes = list(linewidth = 1.5))) +
  ...
  scale_y_continuous(limits = c(1, 100), labels = label_number(big.mark = "")) +
  #scale_y_continuous(limits = c(1, 300), labels = label_number(big.mark = "")) +
  ...
  #scale_x_continuous(limits = c(10, 10000), labels = label_number(big.mark = ""))
  scale_x_log10(limits = c(10, 10000), labels = label_number(big.mark = "")) +
  #scale_x_log10(limits = c(1, 3500), labels = label_number(big.mark = ""))

  # x1000 on x axis
  #geom_vline(xintercept = 12, color = "black", linetype = "dotted", size=0.5) +
...
#save pdf
ggsave("psmc_hifi_x_log_plot_all.x_axis_10k_3500k.pdf", device = cairo_pdf, p1_initial, width = 6, height = 6)

**********above is the code for multiple species from psmc analyses****************  
  
   + # Set x-axis limits between 1 and 100,000
...
# Save the plot
ggsave("with_singleton_non_log_plot_all_5x.pdf", p5x_final, width = 10, height = 8)
print(p1_final) 
