...
layout(matrix(c(1:12),nrow=4,ncol=3,byrow=T),width=c(1,1),height=c(1,1))

#for (group in c("pas_REF_pas","vet_REF_pas","hae_REF_hae","mar_REF_hae")){
    ## single vector in for loop

for (group in c("pas_REF_pas", "vet_REF_pas", "hae_REF_hae", "mar_REF_mar", "mar_REF_hae", "mar_REF_hat")){
    mu <- 3.6e-9
    gen <- 1
    ## key step
