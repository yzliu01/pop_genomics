...
## check the files
#mixedsort(sort(fs::dir_ls(path=".", recurse = 1, fail=TRUE, type = "file", glob = "*100_500G*300hapS*.final.summary")))
...
pdf(paste0("combined.10000Ne_300hapS_5rep_2ei_2ed_2ed_2ei.pdf"),width = 20, height = 12)
...
for (generation in c("100_500G")){
    ...
    for (event in c("2e_i_2e_d","2e_d_2e_i")){
        ## solo axis name plot
        ...
        plot_name <- paste0("10000Ne_300hapS_2E_",generation,"_",event,"_20Chr_15Mb")
        #print(plot_name)
        cat("\n",plot_name,"\n\n")
        
        if (plot_name %in% c("10000Ne_300hapS_2E_100_500G_2i_2i_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e5), ylim=c(1,2.5e4),log="x", xlab="Years ago", ylab="Effective population size",xaxt='n',yaxt='n')
            #log="xy",xlab="Years ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(500,5000,10000,20000),label=c(format(c(500,5000,10000,20000),scientific=FALSE)))
            
        } else if (plot_name %in% c("10000Ne_300hapS_2E_100_500G_2d_2d_20Chr_15Mb")){
            ...
            title(line=2)
            axis(side=2, at=c(500,1250,2500,5000,8000),label=c(format(c(500,1250,2500,5000,8000),scientific=FALSE)))
             
        } else if (plot_name %in% c("10000Ne_300hapS_2E_100_500G_10d_10d_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,1.0e4),log="x", xlab="Years ago", ylab="",xaxt='n',yaxt='n')
            #xlab="Years ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(50,500,2500,5000,8000),label=c(format(c(50,500,2500,5000,8000),scientific=FALSE)))

        } else if (plot_name %in% c("10000Ne_300hapS_2E_100_500G_2d_2i_20Chr_15Mb","10000Ne_300hapS_2E_100_500G_2i_2d_20Chr_15Mb","10000Ne_300hapS_2E_100_500G_2e_i_2e_d_20Chr_15Mb","10000Ne_300hapS_2E_100_500G_2e_d_2e_i_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,1.5e4),log="x", xlab="Years ago", ylab="",xaxt='n',yaxt='n')
            #xlab="Years ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(500,2500,5000,10000),label=c(format(c(500,2500,5000,10000),scientific=FALSE)))
        ...
        ## distance to x axis
        if (plot_name %in% c("10000Ne_300hapS_2E_100_500G_2i_2i_20Chr_15Mb")){
            text(x=c(50,100,500,1000,10000,100000),labels = c(format(lablist.x,scientific=FALSE)),par("usr")[3] - 3500,srt = 38,pos = 3, xpd = TRUE)
        } else if (plot_name %in% c("10000Ne_300hapS_2E_100_500G_2d_2d_20Chr_15Mb","10000Ne_300hapS_2E_100_500G_10d_10d_20Chr_15Mb")) {
            text(x=c(50,100,500,1000,10000,100000),labels = c(format(lablist.x,scientific=FALSE)),par("usr")[3] - 1400,srt = 38,pos = 3, xpd = TRUE)
        } else {
            #,"1000000Ne_300hapS_2E_100_500G_2e_i_2e_d_20Chr_15Mb","1000000Ne_300hapS_2E_100_500G_2e_d_2e_i_20Chr_15Mb"
        ...
        mtext(side=3,text=paste0("10000Ne_300hapS_2E_",generation,"_",event,"_20Chr_15Mb"),cex=0.8,line=1.2)
        ...
        for (rep in c(1:5)){
            ...
            assign(paste0("a",rep), read.table(mixedsort(sort(fs::dir_ls(path=paste0("./ft_sim_10000Ne_300hapS_2E_",generation,"_",event,"_20Chr_15Mb_",rep), recurse = 1, fail=TRUE, type = "file", glob = "*300hapS*.final.summary"))),header=T,sep="\t"))
            #print(head(get(paste0("a",rep)),n=2L))
            #paste0("data_ft_sim_10000Ne_300hapS_2E_",generation,"_",event,"_20Chr_15Mb")
        ...
        x <- 0:100000
        #plot_name <- c("10000Ne_300hapS_2E_1000G_1.1i_20Chr_15Mb","10000Ne_300hapS_2E_1000G_1.5i_20Chr_15Mb","10000Ne_300hapS_2E_1000G_1.9i_20Chr_15Mb","10000Ne_300hapS_2E_1000G_0.5e_d_1.5i_20Chr_15Mb")
        ...
        fx <- function(x) {
            if (all(sapply(c("xxxx"), grepl, plot_name))) {
                ...
                if (x <= 100) {
                    return(5000*(exp(log(2)/100*x)))
                } else if (100 < x & x <=500) {
                    return(5000*(exp(log(2)/100*100))*(exp(log(1/2)/400*(x-100))))
                } else {
                    return(5000*(exp(log(2)/100*100))*(exp(log(1/2)/400*400)))
                }
            } else if (all(sapply(c("100_500G","2e_d_2e_i"), grepl, plot_name))) {
                if (x <= 100) {
                    return(5000*(exp(log(0.5)/100*x)))
                } else if (100 < x & x <=500) {
                    return(5000*(exp(log(0.5)/100*100))*(exp(log(2)/400*(x-100))))
                } else {
                    return(5000*(exp(log(0.5)/100*100))*(exp(log(2)/400*400)))
                }
            } else {
            #    ## return empty value
            #    NULL
