...
#SBATCH --time=01-03:14:00
#SBATCH --error=ft_sim_10K_100K_1MNe_20_80_200_200S_1E_00_01_05_09d_05i_20Chr_15Mb.%A.e.txt
#SBATCH --output=ft_sim_10K_100K_1MNe_20_80_200_200S_1E_00_01_05_09d_05i_20Chr_15Mb.%A.o.txt
#SBATCH --job-name=ftsim_3
...
$fsc -i $par -n 1 -q -s 0 -m --foldedSFS -c 8 -g -G -k 300000000
...
for gen_vcf in `find -print | grep "15Mb-" | grep '.vcf$'`;do
    plink --vcf $gen_vcf --double-id --allow-extra-chr --set-missing-var-ids @:# --indep-pairwise 50 10 0.1 --out $gen_vcf.plink_ld01
    done
...
for gen_vcf in `find -print | grep "15Mb-" | grep '.vcf$'`;do
    plink --vcf $gen_vcf --double-id --allow-extra-chr --set-missing-var-ids @:# --extract $gen_vcf.plink_ld01.prune.in --out $gen_vcf.plink_ld01.pruned --keep-allele-order --recode vcf
    done
...
bcftools query -l $vcf | grep 'G_1' | awk '{split($0,a,"_G");print $1,"pop"}' > pop_file

conda activate bioproject

/home/yzliu/bin/easySFS/easySFS.py -i $vcf -p pop_file -a -f --preview
## choose number of samples with high SNPs
#pop
#(2, 35800)      (3, 53701)      (4, 62222)      (5, 66054)      (6, 67667)      (7, 68296)      (8, 68523)      (9, 68597)      (10, 68620)     (11, 68627)     (12, 68630)     (13, 68631)     (14, 68631)      (15, 68631)     (16, 68631)     (17, 68631)     (18, 68631)     (19, 68631)     (20, 68631)

## correct way ##
/home/yzliu/bin/easySFS/easySFS.py -i $vcf -p ../pop_file -a -f --proj 20
