...
#SBATCH --error=1_gatk_REF1_2_3_bee_pools_markDup_index.new_pasVet.%A_%a.e.txt
#SBATCH --output=1_gatk_REF1_2_3_bee_pools_markDup_index.new_pasvet.%A_%a.o.txt
#SBATCH --job-name=pasVet_markDup_index
...
SORTED_BAM=$(ls Bomvet.REF_BomPas.sort.bam | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p) # forward sequence
MARKED_BAM=${SORTED_BAM/.sort.bam/.sort.marked_dups.new.bam}
...
samtools stats $MARKED_BAM > ./bam_stats/samtools_stats/$MARKED_BAM.stat

## stats
# "\n######  bamtools stats: $MARKED_BAM ######" >> bamtools_stats_marked_dups_4_pools.txt
bamtools stats -in $MARKED_BAM > ./bam_stats/bamtools_stats/$MARKED_BAM.stat
