...
##SBATCH --mem 160g # genome size 300-400Mb
##SBATCH --time=02:15:00
#SBATCH --mem 500g # genome size ChoBru 14Gb
#SBATCH --time=08:15:00
#SBATCH --array=1-4%4
#SBATCH --error=busco_orthodb_ChoPar_assembly.%A_%a.e
#SBATCH --output=busco_orthodb_ChoPra_assembly.%A_%a.o
#SBATCH --job-name=busco_orthodb_ChoPra_assembly
...
## run busco
# fasta files
assembly_fa_stats=/home/yzliu/eDNA/faststorage/yzliu/DK_proj/data/bee_proj_data/hifi/Chorthippus_parallelus/filtered/assembly_fa_stats
## make array jobs
fa_list=$(ls $assembly_fa_stats/Chorthippus_parallelus.pl*asm.bp.p_ctg.gfa.fa | tr ' ' '\n' | sed -n ${SLURM_ARRAY_TASK_ID}p)
#fa_list=$(ls $assembly_fa_stats/Chorthippus_parallelus.pl_3.asm.bp.p_ctg.gfa.fa | tr ' ' '\n' | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
busco -i $fa_list -m genome -c 20 --lineage_dataset insecta_odb10 --restart --out_path $hifiasm_busco_out
## Chorthippus_parallelus.pl_0.asm.bp.p_ctg.gfa.fa
## Chorthippus_parallelus.pl_0.with_fail.asm.bp.p_ctg.gfa.fa
...
## install busco and use the right database
## https://busco.ezlab.org/busco_userguide.html
mamba create -n busco_v5.7.1 -c bioconda busco=5.7.1

## A full list of available datasets can be viewed 
## after busco_downloads are downloaded in the directory where you run the comand
busco --list-datasets
...
stats for Chorthippus_parallelus.pl_2.asm.bp.p_ctg.gfa.fa
...
# Summarized benchmarking in BUSCO notation for file /home/yzliu/eDNA/faststorage/yzliu/DK_proj/data/bee_proj_data/hifi/Chorthippus_parallelus/filtered/assembly_fa_stats/Chorthippus_parallelus.pl_2.with_fail.asm.bp.p_ctg.gfa.fa
