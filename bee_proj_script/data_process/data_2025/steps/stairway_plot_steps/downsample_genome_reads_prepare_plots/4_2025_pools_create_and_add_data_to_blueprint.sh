...
for compiled_sfs in `ls -t *_dipS.DP_1_3_5_7_10x.sfs.blueprint.txt`
do
echo $compiled_sfs
#head -1 $compiled_sfs
...
for species in {Cerryb_50,Meglea_50,Rutmac_50};do
    for bp_file in `cat ./templates/"$species"_dipS.DP_1_3_5_7_10x.sfs.blueprint.folder_name.list`;do
        #echo $bp_file by using templates
        cat ./templates/template."$species".no_singleton_sfs_nx_1500x.sm_genic.blueprint > $bp_file
...
for i in {1..5};do
    for species in {Cerryb_50,Meglea_50,Rutmac_50};do
        ...
        sfs3=`cat ./templates/"$species"_dipS.DP_1_3_5_7_10x.sfs.blueprint.folder_name.list | awk -F " " -v var=$i 'FNR==var {print $NF}'`
        ...
        sfs4=`cat ./templates/"$species"_dipS.DP_1_3_5_7_10x.sfs.blueprint.txt | awk -F " " -v var=$i 'FNR==var {$NF=$(NF-1)="";print}' | sed 's/[[:blank:]]*$//'`
