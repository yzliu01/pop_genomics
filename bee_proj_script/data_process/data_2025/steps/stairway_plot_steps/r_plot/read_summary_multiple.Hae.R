...
# Read all files into a list of dataframes
summary_list <- lapply(c(sorted_files,no_called_sites), read_summary)
...
plot_summary_legend <- function(data) {
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    geom_text(data = no_called_sites[i,])
...
pdf("AndHae_downsample_stairway_plot.pdf", width = 30, height = 15)
