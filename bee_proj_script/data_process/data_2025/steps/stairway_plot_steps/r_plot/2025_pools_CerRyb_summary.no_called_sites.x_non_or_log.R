...
plot_summary_legend <- function(data) {
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_continuous(limits= c(1,8000000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
    ...
    annotate("text", x = 250000, y = 5000000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 2.7, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 250000, y = 4000000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 2.7, hjust = 0, fontface = "plain", color = "black") +
