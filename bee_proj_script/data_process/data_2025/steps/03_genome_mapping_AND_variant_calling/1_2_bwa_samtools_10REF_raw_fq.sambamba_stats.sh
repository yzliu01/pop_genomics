...
#SBATCH --error=1_bwa_samtools_10REF_2step_sambamba_markDup_stats.%A_%a.e
#SBATCH --output=1_bwa_samtools_10REF_2step_sambamba_markDup_stats.%A_%a.o
#SBATCH --job-name=1_bwa_samtools_10REF_2step_sambamba_markDup_stats
...
seq1=$(ls *R1_001.fastq.gz | sed -n ${SLURM_ARRAY_TASK_ID}p) # forward sequence
seq2=$(ls *R2_001.fastq.gz | sed -n ${SLURM_ARRAY_TASK_ID}p) # reverse sequence
...
bwa mem -t 20 -R $ReadGroup $REF_DIR/$REF1 $FASTQ_RAW/$seq1 $FASTQ_RAW/$seq2 | samtools sort  -@ 8 -m 100G -o $OUT_BAM/$File1.$OUT_NAME".sort.bam"
