...
# Find the y-value where x ≈ 100 (adjust tolerance if needed)
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_log10(limits= c(100,400000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
    ...
    annotate("text", x = 10, y = 20000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 10, y = 10000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
...
## 10 pools
pdf("BomMaj_stairway_plot.called_sites_snps.final.xy_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list2, nrow = 1)
dev.off()
