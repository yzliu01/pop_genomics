...
# Find the y-value where x ≈ 100 (adjust tolerance if needed)
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_log10(limits= c(100,3500000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
...
## 10 pools
pdf("OchPle_stairway_plot.called_sites_snps.final.xy_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list7, nrow = 1)
dev.off()
