...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/SteMel_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/SteMel_snp_number.DP_1_1.5_2_2.5_3x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./SteMel_REF_SteMel.DP_1x/SteMel_REF_SteMel.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "SteMel_REF_SteMel.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./SteMel_REF_SteMel.DP_1x/SteMel_REF_SteMel.DP_1x.final.summary"
#[2] "./SteMel_REF_SteMel.DP_1.5x/SteMel_REF_SteMel.DP_1.5x.final.summary"
#[3] "./SteMel_REF_SteMel.DP_2x/SteMel_REF_SteMel.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("SteMel_REF_SteMel",".final.summary"),
    replacement=c("SteMel",""),
    vectorize=F
  )
...
## obtain only one label
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_log10(limits= c(100,5000000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
...
combined_plot <- wrap_plots(c(combined_plot1, combined_plot9, combined_plot10, 
...
# Ensure combined_plot1, combined_plot2, and combined_plot3 are not empty
# and contain the correct plots. If they are lists of plots, we need to wrap them.
combined_plot <- wrap_plots(c(combined_plot1_new, combined_plot9_new, combined_plot10_new, 
                            combined_plot3_new, combined_plot5_new,
                            combined_plot2_new,
                            combined_plot6_new,
                            combined_plot7_new,combined_plot8_new,
                            combined_plot4_new)) + plot_layout(ncol = 5, nrow = 2)
