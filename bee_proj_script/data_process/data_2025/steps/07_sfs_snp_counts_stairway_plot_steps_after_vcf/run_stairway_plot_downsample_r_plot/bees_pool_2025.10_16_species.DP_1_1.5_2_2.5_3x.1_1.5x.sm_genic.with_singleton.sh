...
##SBATCH --time=4:00:00
#SBATCH --error=bees_pool_2025.10_16_species.DP_1_1.5_2_2.5_3x.1_1.5x.sm_genic.with_singleton.%A_%a.e
#SBATCH --output=bees_pool_2025.10_16_species.DP_1_1.5_2_2.5_3x.1_1.5x.sm_genic.with_singleton.%A_%a.o
#SBATCH --job-name=bees_pool_2025.10_16_species.DP_1_1.5_2_2.5_3x.1_1.5x.sm_genic.with_singleton
...
blueprint_file_dir=./stairway_plot_blueprint/2025_pools
## check declining PasVet first
blueprint_file=`ls -t $blueprint_file_dir/*with_singleton.blueprint | head -80 | sort -V`
...
## run batch files with for loop (slow)
...
## run batch files in array jobs to plot (fast)
run_blueprint_plot_sh=$(ls -t $blueprint_file_dir/*with_singleton.blueprint.sh | head -80 | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p)
