...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/pool_downsample_reads/templates/Mar_40_number_called_sites_downsample_reads_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/pool_downsample_reads/templates/Andmar_snp_number.P_reads_01_02_03_04_05_06_07_08_09_10.DP_1_3_5_7_10x.txt"
...
summary_files <- list.files(path = ".", pattern = "AndMar_REF.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("AndMar_REF_AndMar",".final.summary"),
    replacement=c("AndMar",""),
    vectorize=F
  )
...
pdf("AndMar_downsample_reads_stairway_plot.called_sites_snps.pdf", width = 30, height = 15)
...
input_pdfs <- c(
    "AndHae_downsample_reads_stairway_plot.called_sites_snps.pdf", 
    "AndMar_downsample_reads_stairway_plot.called_sites_snps.pdf"
    )  

# Output merged PDF filename
output_pdf <- "combined_read_downsample_plot.pdf"
