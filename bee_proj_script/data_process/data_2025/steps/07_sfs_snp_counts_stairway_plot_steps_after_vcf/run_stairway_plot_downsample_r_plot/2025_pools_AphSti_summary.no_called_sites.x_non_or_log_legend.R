...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/AphSti_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/AphSti_snp_number.DP_1_1.5_2_2.5_3x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./AphSti_REF_AphSti.DP_1x/AphSti_REF_AphSti.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "AphSti_REF_AphSti.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
# [1] "./3_test_pools/AphSti_REF_AphSti.DP_1x/AphSti_REF_AphSti.DP_1x.final.summary"  
# [2] "./3_test_pools/AphSti_REF_AphSti.DP_3x/AphSti_REF_AphSti.DP_3x.final.summary"  
# [3] "./3_test_pools/AphSti_REF_AphSti.DP_5x/AphSti_REF_AphSti.DP_5x.final.summary"  
# [4] "./3_test_pools/AphSti_REF_AphSti.DP_7x/AphSti_REF_AphSti.DP_7x.final.summary"  
# [5] "./3_test_pools/AphSti_REF_AphSti.DP_10x/AphSti_REF_AphSti.DP_10x.final.summary"
# [6] "./AphSti_REF_AphSti.DP_1x/AphSti_REF_AphSti.DP_1x.final.summary"               
# [7] "./AphSti_REF_AphSti.DP_1.5x/AphSti_REF_AphSti.DP_1.5x.final.summary"           
# [8] "./AphSti_REF_AphSti.DP_2x/AphSti_REF_AphSti.DP_2x.final.summary"               
# [9] "./AphSti_REF_AphSti.DP_2.5x/AphSti_REF_AphSti.DP_2.5x.final.summary"           
# [10] "./AphSti_REF_AphSti.DP_3x/AphSti_REF_AphSti.DP_3x.final.summary" 
...
#[1] "./AphSti_REF_AphSti.DP_1x/AphSti_REF_AphSti.DP_1x.final.summary"
#[2] "./AphSti_REF_AphSti.DP_1.5x/AphSti_REF_AphSti.DP_1.5x.final.summary"
#[3] "./AphSti_REF_AphSti.DP_2x/AphSti_REF_AphSti.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("AphSti_REF_AphSti",".final.summary"),
    replacement=c("AphSti",""),
    vectorize=F
  )
...
# Find the y-value where x ≈ 100 (adjust tolerance if needed)
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_log10(limits= c(100,4000000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
    ...
    annotate("text", x = 10, y = 1500000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
...
#pdf("AphSti_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("AphSti_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list, nrow = 1)

## 10 pools
pdf("AphSti_stairway_plot.called_sites_snps.final.xy_log.label.pdf", width = 20, height = 4)
