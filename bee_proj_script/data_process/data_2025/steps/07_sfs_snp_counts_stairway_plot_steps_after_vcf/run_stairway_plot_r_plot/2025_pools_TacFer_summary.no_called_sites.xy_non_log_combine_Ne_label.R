...
#for species in {TacFer,AgeAln,AmpSol,EriInt,EriPer,GerLac,LasMor,LepDol,MalBip,MysLon,PhoAtr,RhaFul,TacFer,ThoDec,XesC-n};do

## 16 pools
## for DP_1_1.5x
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/TacFer_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/TacFer_snp_number.DP_1_1.5x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./TacFer_REF_TacFer.DP_1x/TacFer_REF_TacFer.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "TacFer_REF_TacFer.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./TacFer_REF_TacFer.DP_1x/TacFer_REF_TacFer.DP_1x.final.summary"
#[2] "./TacFer_REF_TacFer.DP_1.5x/TacFer_REF_TacFer.DP_1.5x.final.summary"
#[3] "./TacFer_REF_TacFer.DP_2x/TacFer_REF_TacFer.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("TacFer_REF_TacFer",".final.summary"),
    replacement=c("TacFer",""),
    vectorize=F
  )
...
#plot_list23 <- lapply(summary_list, plot_summary)

plot_list23 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list232 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list23 <- append(plot_list23,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list23 <- append(plot_list23,list(plot_summary(summary_list[[i]])))
...
#pdf("TacFer_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("TacFer_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list23, nrow = 1)

## 10 pools
pdf("TacFer_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list23, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot23 <- marrangeGrob(grobs = plot_list23,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


## 5 rows (1x, 1.5x, 2x, 2.5x, 3x)
combined_plot23 <- marrangeGrob(grobs = plot_list23,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))


## 2 rows (1x, 1.5x)
combined_plot23_new <- marrangeGrob(grobs = plot_list23[1:2], ncol = 1, nrow = 2, top=NULL,
