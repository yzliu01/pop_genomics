...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/CerRyb_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/CerRyb_snp_number.DP_1_1.5_2_2.5_3x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./CerRyb_REF_CerRyb.DP_1x/CerRyb_REF_CerRyb.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "CerRyb_REF_CerRyb.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./CerRyb_REF_CerRyb.DP_1x/CerRyb_REF_CerRyb.DP_1x.final.summary"
#[2] "./CerRyb_REF_CerRyb.DP_1.5x/CerRyb_REF_CerRyb.DP_1.5x.final.summary"
#[3] "./CerRyb_REF_CerRyb.DP_2x/CerRyb_REF_CerRyb.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("CerRyb_REF_CerRyb",".final.summary"),
    replacement=c("CerRyb",""),
    vectorize=F
  )
...
# Find the y-value where x ≈ 100 (adjust tolerance if needed)
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    annotate("text", x = 10, y = 3500000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 10, y = 3200000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
...
plot_list3 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list3 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list3 <- append(plot_list3,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list3 <- append(plot_list3,list(plot_summary(summary_list[[i]])))
...
#pdf("CerRyb_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("CerRyb_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list, nrow = 1)

## 10 pools
pdf("CerRyb_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list3, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot3 <- marrangeGrob(grobs = plot_list3,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


combined_plot3 <- marrangeGrob(grobs = plot_list3,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))

## 3 rows (1x, 1.5x, 2x)
combined_plot3_new <- marrangeGrob(grobs = plot_list3[1:3],ncol = 1, nrow = 3, top=NULL,
