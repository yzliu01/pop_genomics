...
#for species in {RhaFul,AgeAln,AmpSol,EriInt,EriPer,GerLac,LasMor,LepDol,MalBip,MysLon,PhoAtr,RhaFul,TacFer,ThoDec,XesC-n};do

## 16 pools
## for DP_1_1.5x
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/RhaFul_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/RhaFul_snp_number.DP_1_1.5x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./RhaFul_REF_RhaFul.DP_1x/RhaFul_REF_RhaFul.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "RhaFul_REF_RhaFul.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./RhaFul_REF_RhaFul.DP_1x/RhaFul_REF_RhaFul.DP_1x.final.summary"
#[2] "./RhaFul_REF_RhaFul.DP_1.5x/RhaFul_REF_RhaFul.DP_1.5x.final.summary"
#[3] "./RhaFul_REF_RhaFul.DP_2x/RhaFul_REF_RhaFul.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("RhaFul_REF_RhaFul",".final.summary"),
    replacement=c("RhaFul",""),
    vectorize=F
  )
...
#plot_list22 <- lapply(summary_list, plot_summary)

plot_list22 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list222 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list22 <- append(plot_list22,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list22 <- append(plot_list22,list(plot_summary(summary_list[[i]])))
...
#pdf("RhaFul_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("RhaFul_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list22, nrow = 1)

## 10 pools
pdf("RhaFul_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list22, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot22 <- marrangeGrob(grobs = plot_list22,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


## 5 rows (1x, 1.5x, 2x, 2.5x, 3x)
combined_plot22 <- marrangeGrob(grobs = plot_list22,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))


## 2 rows (1x, 1.5x)
combined_plot22_new <- marrangeGrob(grobs = plot_list22[1:2], ncol = 1, nrow = 2, top=NULL,
