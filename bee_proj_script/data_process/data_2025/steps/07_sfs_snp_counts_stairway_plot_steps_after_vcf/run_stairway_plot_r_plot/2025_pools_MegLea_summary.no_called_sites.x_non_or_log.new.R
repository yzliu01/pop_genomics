...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/MegLea_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/MegLea_snp_number.DP_1_1.5_2_2.5_3x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./MegLea_REF_MegLea.DP_1x/MegLea_REF_MegLea.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "MegLea_REF_MegLea.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./MegLea_REF_MegLea.DP_1x/MegLea_REF_MegLea.DP_1x.final.summary"
#[2] "./MegLea_REF_MegLea.DP_1.5x/MegLea_REF_MegLea.DP_1.5x.final.summary"
#[3] "./MegLea_REF_MegLea.DP_2x/MegLea_REF_MegLea.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("MegLea_REF_MegLea",".final.summary"),
    replacement=c("MegLea",""),
    vectorize=F
  )
...
# Find the y-value where x ≈ 100 (adjust tolerance if needed)
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_continuous(limits= c(1,700000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
    ...
    annotate("text", x = 10, y = 600000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 10, y = 550000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
...
#plot_list5 <- lapply(summary_list, plot_summary)

plot_list5 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list52 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list5 <- append(plot_list5,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list5 <- append(plot_list5,list(plot_summary(summary_list[[i]])))
...
#pdf("MegLea_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("MegLea_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list5, nrow = 1)

## 10 pools
pdf("MegLea_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list5, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot5 <- marrangeGrob(grobs = plot_list5,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


combined_plot5 <- marrangeGrob(grobs = plot_list5,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))

## 3 rows (1x, 1.5x, 2x)
combined_plot5_new <- marrangeGrob(grobs = plot_list5[1:3],ncol = 1, nrow = 3, top=NULL,
