...
#for species in {GerLac,AgeAln,AmpSol,EriInt,EriPer,GerLac,LasMor,LepDol,MalBip,MysLon,PhoAtr,RhaFul,TacFer,ThoDec,XesC-n};do

## 16 pools
## for DP_1_1.5x
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/GerLac_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/GerLac_snp_number.DP_1_1.5x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./GerLac_REF_GerLac.DP_1x/GerLac_REF_GerLac.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "GerLac_REF_GerLac.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./GerLac_REF_GerLac.DP_1x/GerLac_REF_GerLac.DP_1x.final.summary"
#[2] "./GerLac_REF_GerLac.DP_1.5x/GerLac_REF_GerLac.DP_1.5x.final.summary"
#[3] "./GerLac_REF_GerLac.DP_2x/GerLac_REF_GerLac.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("GerLac_REF_GerLac",".final.summary"),
    replacement=c("GerLac",""),
    vectorize=F
  )
...
## obtain only one label
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_continuous(limits= c(100,2500000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
...
#plot_list16 <- lapply(summary_list, plot_summary)

plot_list16 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list162 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list16 <- append(plot_list16,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list16 <- append(plot_list16,list(plot_summary(summary_list[[i]])))
...
#pdf("GerLac_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("GerLac_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list16, nrow = 1)

## 10 pools
pdf("GerLac_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list16, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot16 <- marrangeGrob(grobs = plot_list16,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


## 5 rows (1x, 1.5x, 2x, 2.5x, 3x)
combined_plot16 <- marrangeGrob(grobs = plot_list16,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))


## 2 rows (1x, 1.5x)
combined_plot16_new <- marrangeGrob(grobs = plot_list16[1:2], ncol = 1, nrow = 2, top=NULL,
