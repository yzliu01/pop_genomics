...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/EphDan_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/EphDan_snp_number.DP_1_1.5_2_2.5_3x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./EphDan_REF_EphDan.DP_1x/EphDan_REF_EphDan.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "EphDan_REF_EphDan.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./EphDan_REF_EphDan.DP_1x/EphDan_REF_EphDan.DP_1x.final.summary"
#[2] "./EphDan_REF_EphDan.DP_1.5x/EphDan_REF_EphDan.DP_1.5x.final.summary"
#[3] "./EphDan_REF_EphDan.DP_2x/EphDan_REF_EphDan.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("EphDan_REF_EphDan",".final.summary"),
    replacement=c("EphDan",""),
    vectorize=F
  )
...
#plot_list4 <- lapply(summary_list, plot_summary)

plot_list4 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list42 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list4 <- append(plot_list4,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list4 <- append(plot_list4,list(plot_summary(summary_list[[i]])))
...
#pdf("EphDan_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("EphDan_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list4, nrow = 1)

## 10 pools
pdf("EphDan_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list4, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot4 <- marrangeGrob(grobs = plot_list4,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


combined_plot4 <- marrangeGrob(grobs = plot_list4,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))

## 3 rows (1x, 1.5x, 2x)
combined_plot4_new <- marrangeGrob(grobs = plot_list4[1:3],ncol = 1, nrow = 3, top=NULL,
