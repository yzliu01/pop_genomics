...
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/Meglea_snp_number.DP_1_3_5_7_10x.txt"
...
plot_summary_legend <- function(data) {
  ggplot(data, aes(x = year)) +  # Adjust column names
    #scale_x_continuous(limits= c(10,10000),labels = function(year) format(year, scientific = FALSE)) +
    scale_x_log10(limits= c(10,100000),labels = function(year) format(year, scientific = FALSE)) +
    ...
    annotate("text", x = 20, y = 12000000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 2.7, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 20, y = 10000000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 2.7, hjust = 0, fontface = "plain", color = "black") +
...
plot_summary <- function(data) {
  ggplot(data, aes(x = year, y = Ne_median)) +  # Adjust column names
    #scale_x_continuous(limits= c(10, 10000),labels = function(year) format(year, scientific = FALSE)) +
    scale_x_log10(limits= c(10,50000),labels = function(year) format(year, scientific = FALSE)) +
    scale_y_continuous(limits= c(1, 25000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
    ...
    # Add annotation for `no_called_sites`
    annotate("text", x = 200, y = 22500, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 2.7, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 200, y = 20000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 2.7, hjust = 0, fontface = "plain", color = "black") +
...
combined_plot2 <- marrangeGrob(grobs = plot_list,ncol = 5, nrow = 1, top=NULL,
