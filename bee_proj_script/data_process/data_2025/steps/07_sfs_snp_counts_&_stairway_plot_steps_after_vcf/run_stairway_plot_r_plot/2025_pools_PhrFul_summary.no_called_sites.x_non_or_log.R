...
site_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/PhrFul_50_number_called_sites_across_genome.txt"
no_called_sites <- read.table(site_file, header = FALSE)
snp_file <- "/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/stairway_plot_blueprint/2025_pools/templates/PhrFul_snp_number.DP_1_1.5_2_2.5_3x.txt"
snp_number <- read.table(snp_file, header = FALSE)
...
## ./PhrFul_REF_PhrFul.DP_1x/PhrFul_REF_PhrFul.DP_1x.final.summary
summary_files <- list.files(path = ".", pattern = "PhrFul_REF_PhrFul.*DP_.*x.*final\\.summary$", recursive = TRUE, full.names = TRUE)
...
#[1] "./PhrFul_REF_PhrFul.DP_1x/PhrFul_REF_PhrFul.DP_1x.final.summary"
#[2] "./PhrFul_REF_PhrFul.DP_1.5x/PhrFul_REF_PhrFul.DP_1.5x.final.summary"
#[3] "./PhrFul_REF_PhrFul.DP_2x/PhrFul_REF_PhrFul.DP_2x.final.summary" 
...
read_summary <- function(file, index) {
  ...
  data$file <- stri_replace_all_regex(
    ...
    #replacement=c("","P.DP",""),
    pattern=c("PhrFul_REF_PhrFul",".final.summary"),
    replacement=c("PhrFul",""),
    vectorize=F
  )
...
# Find the y-value where x ≈ 100 (adjust tolerance if needed)
  ...
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    scale_y_continuous(limits= c(1,15000000),labels = function(Ne_median) format(Ne_median, scientific = FALSE)) +
    ...
    annotate("text", x = 10, y = 12500000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 10, y = 11250000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
...
#plot_list8 <- lapply(summary_list, plot_summary)

plot_list8 <- list() # intialize empty list
...
for (i in 1:length(sorted_files)){
    ...
    #plot_list82 <- list(plot_summary_legend(summary_list[[i]])) # use [[i]]
    plot_list8 <- append(plot_list8,list(plot_summary_legend(summary_list[[i]]))) # use [[i]]

    } else {
    
    plot_list8 <- append(plot_list8,list(plot_summary(summary_list[[i]])))
...
#pdf("PhrFul_stairway_plot.called_sites_snps.pdf", width = 20, height = 6)
## 3 pools test
#pdf("PhrFul_stairway_plot.called_sites_snps.x_log.pdf", width = 20, height = 6)
#grid.arrange(grobs = plot_list8, nrow = 1)

## 10 pools
pdf("PhrFul_stairway_plot.called_sites_snps.final.x_log.label.pdf", width = 20, height = 4)
grid.arrange(grobs = plot_list8, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot8 <- marrangeGrob(grobs = plot_list8,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


combined_plot8 <- marrangeGrob(grobs = plot_list8,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))

## 3 rows (1x, 1.5x, 2x)
combined_plot8_new <- marrangeGrob(grobs = plot_list8[1:3],ncol = 1, nrow = 3, top=NULL,
