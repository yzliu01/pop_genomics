...
plot_summary_legend <- function(data) {
  ggplot(data, aes(x = year)) +  # Adjust column names
    ...
    annotate("text", x = 30, y = 17500000, label = paste("Site No: ", unique(data$no_called_sites)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
    annotate("text", x = 30, y = 16250000, label = paste("SNP No: ", unique(data$snp_number)),
    size = 3, hjust = 0, fontface = "plain", color = "black") +
...
## 10 pools
pdf("NotGla_stairway_plot.called_sites_snps.final.x_log.new.pdf", width = 20, height = 6)
grid.arrange(grobs = plot_list, nrow = 1)
dev.off()

# Combine plots with patchwork
combined_plot6 <- marrangeGrob(grobs = plot_list,ncol = 5, nrow = 1, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 1, byrow = FALSE))


combined_plot6 <- marrangeGrob(grobs = plot_list,ncol = 1, nrow = 5, top=NULL,
                            layout_matrix = matrix(seq_len(5), nrow = 5, byrow = TRUE))

## 3 rows (1x, 1.5x, 2x)
combined_plot6_new <- marrangeGrob(grobs = plot_list[1:3],ncol = 1, nrow = 3, top=NULL,
