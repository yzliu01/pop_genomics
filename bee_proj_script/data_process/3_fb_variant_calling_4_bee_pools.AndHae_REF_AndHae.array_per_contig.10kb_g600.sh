...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.AndHae_REF_AndHae.array_per_contig_8.10kb_g600.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.AndHae_REF_AndHae.array_per_contig_8.10kb_g600.%A_%a.o
...
contig_region=$(ls $fasta_generate_regions/per_chr/iyAndHaem1_1.md_chr.fa.10kbp.regions.fb* | sed -n ${SLURM_ARRAY_TASK_ID}p)
# iyAndHaem1_1.md_chr.fa.10kbp.regions.fb.chr5
contig_name=$(cut -f 1 $REF_DIR/iyAndHaem1_1.md_chr.fa.fai | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
freebayes-parallel $contig_region 12 --fasta-reference $REF \
    ...
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_AndHae_REF_AndHae/"$BAM2VCF_NAME"_"$contig_name".qual_20.g.vcf
