...
SAMPLE=Bompas.REF_BomHypn.sort.bam
...
freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    ...
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomHyp/"$BAM2VCF_NAME_200"_"$contig_regions_order".qual_20.g.vcf

freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    --ploidy 68 --pooled-discrete --genotype-qualities --use-best-n-alleles 4 \
    --bam $BAM_DIR/$SAMPLE -g 400 --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomHyp/"$BAM2VCF_NAME_400"_"$contig_regions_order".qual_20.g.vcf
