...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomPas.10kb_g1500x.chr_regions.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomPas.10kb_g1500x.chr_regions.%A_%a.o
...
SAMPLE=Bompas.REF_BomPas.sort.marked_dups.new.bam
...
freebayes --region ./BomPas/iyBomPasc1_1.md_chr.fa.10kbp.regions.01.fb --fasta-reference $REF \
    ...
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomPas/"$BAM2VCF_NAME""$contig_regions_order".qual_20.g.vcf

freebayes --region $contig_regions --fasta-reference $REF \
    --ploidy 68 --pooled-discrete --genotype-qualities --report-monomorphic --use-best-n-alleles 4 \
    --bam $BAM_DIR/$SAMPLE -g 1500 --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" 
