...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.AndHae_New_REF_AndHae.100kb_g1500x.chr_regions.short.likelihood.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.AndHae_New_REF_AndHae.100kb_g1500x.chr_regions.short.likelihood.%A_%a.o
#SBATCH --job-name=3_fb_variant_calling_4_bee_pools.AndHae_New_REF_AndHae.short.likelihood
...
mkdir $VCF_OUT_DIR/fb_per_region_AndHae_New_REF_AndHae_likelihood
#cd $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomPas
cd $VCF_OUT_DIR/fb_per_region_AndHae_New_REF_AndHae_likelihood
...
freebayes --region $Each_Region_Ref --fasta-reference $REF \
    --ploidy 78 --genotype-qualities --use-best-n-alleles 3 --min-alternate-fraction 0 --min-alternate-count 2 \
    --bam $BAM_DIR/$SAMPLE -g 1500 --strict-vcf --gvcf --report-genotype-likelihood-max --report-all-haplotype-alleles \
    > $VCF_OUT_DIR/fb_per_region_AndHae_New_REF_AndHae_likelihood/"Andhae.New_REF_AndHae.100kb_1500x_region_"${SLURM_ARRAY_TASK_ID}.likelihood.g.vcf
