...
#SBATCH --error=1_REF1_mapping_sort_markDup_bee_4_pools_3_4.%A_%a.e.txt
#SBATCH --output=1_REF1_mapping_sort_markDup_bee_4_pools_3_4.%A_%a.o.txt
#SBATCH --job-name=1_REF1_mapping_bee_pools_3_4
...
echo -e "aligning: $seq1 $seq2\n" >> $script_path/bee_REF1_aligning_sorting_3_4.log
bwa mem -t 6 -R $ReadGroup $REF_DIR/$REF1 $FASTQ_CLEAN/$seq1 $FASTQ_CLEAN/$seq2 | samtools sort -m 650G -o $OUT_BAM/$File1
#bwa mem -t 6 -R $ReadGroup $REF $SEQDIR/$seq1 $SEQDIR/$seq2 | samtools view -b -@ 6 -o $File1
#samtools sort -@ 6 -m 40G -o $File2 $File1

## indexing sorted
echo -e "\nindexing: $File1\n" >> $script_path/bee_REF1_sort_marked_bam_index_3_4.log
