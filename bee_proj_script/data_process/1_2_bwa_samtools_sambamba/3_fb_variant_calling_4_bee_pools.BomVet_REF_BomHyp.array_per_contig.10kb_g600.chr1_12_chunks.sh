...
##SBATCH --time=3-14:04:00
#SBATCH --error=3_fb_variant_calling_4_bee_pools.BomVet_REF_BomHyp.array_per_contig_53.10kb_g600.chr1_12_chunks.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.BomVet_REF_BomHyp.array_per_contig_53.10kb_g600.chr1_12_chunks.%A_%a.o
...
fasta_generate_chunks=/home/yzliu/eDNA/faststorage/yzliu/DK_proj/data/ref_genome/fasta_generate_regions/chr_chunks
cd $fasta_generate_chunks

## a problem with the output names

contig_chunks=$(ls $fasta_generate_chunks/BomHypn/iy*60_chunks.*[0-9].fb* | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
freebayes-parallel $contig_chunks 20 --fasta-reference $REF \
    --ploidy 58 --pooled-discrete --genotype-qualities --use-best-n-alleles 4 \
    --bam $BAM_DIR/$SAMPLE -g 600 --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_BomVet_REF_BomHyp/"$BAM2VCF_NAME"_"${SLURM_ARRAY_TASK_ID}".qual_20.g.vcf
