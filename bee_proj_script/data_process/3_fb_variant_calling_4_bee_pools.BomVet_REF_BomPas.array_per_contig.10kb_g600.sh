...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.BomVet_REF_BomPas.array_per_contig_18.10kb_g600.%A.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.BomVet_REF_BomPas.array_per_contig_18.10kb_g600.%A.o
...
contig_region=$(ls $fasta_generate_regions/per_chr/iyBomPasc1_1.md_chr.fa.10kbp.regions.fb* | sed -n ${SLURM_ARRAY_TASK_ID}p)
# iyAndHaem1_1.md_chr.fa.10kbp.regions.fb.chr5
contig_name=$(cut -f 1 $REF_DIR/iyBomPasc1_1.md_chr.fa.fai | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
freebayes-parallel $contig_region 12 --fasta-reference $REF \
    ...
    --bam $BAM_DIR/$SAMPLE -g 600 --strict-vcf --gvcf > $VCF_OUT_DIR/fb_per_contig_BomVet_REF_BomPas/"$BAM2VCF_NAME"_"$contig_name".g.vcf
