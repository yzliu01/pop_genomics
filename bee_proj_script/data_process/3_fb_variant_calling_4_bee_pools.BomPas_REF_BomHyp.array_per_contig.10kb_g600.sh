...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomHyp.array_per_contig_18.10kb_g600.%A.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomHyp.array_per_contig_18.10kb_g600.%A.o
#SBATCH --job-name=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomHyp
...
contig_region=$(ls $fasta_generate_regions/per_chr/iyBomHypn_7925v1_2.md_chr.fa.10kbp.regions.fb* | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
freebayes-parallel $contig_region 12 --fasta-reference $REF \
    ...
    --bam $BAM_DIR/$SAMPLE -g 600 --strict-vcf --gvcf > $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomHyp/"$BAM2VCF_NAME"_"$contig_name".g.vcf
