...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.AndMar_REF_AndHae.10kb_g200_400.chr_regions_all_sites.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.AndMar_REF_AndHae.10kb_g200_400.chr_regions_all_sites.%A_%a.o
...
freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    ...
    --bam $BAM_DIR/$SAMPLE -g 200 --report-all-haplotype-alleles --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_AndMar_REF_AndHae/"$BAM2VCF_NAME_200""$contig_regions_order".all_sites.qual_20.g.vcf

freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    --ploidy 80 --pooled-discrete --genotype-qualities --use-best-n-alleles 4 \
    --bam $BAM_DIR/$SAMPLE -g 400 --report-all-haplotype-alleles --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_AndMar_REF_AndHae/"$BAM2VCF_NAME_400""$contig_regions_order".all_sites.qual_20.g.vcf
