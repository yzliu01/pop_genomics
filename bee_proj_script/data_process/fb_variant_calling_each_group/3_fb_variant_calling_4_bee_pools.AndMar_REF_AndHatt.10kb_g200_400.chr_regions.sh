...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.AndMar_REF_AndHatt.10kb_g200_400.chr_regions.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.AndMar_REF_AndHatt.10kb_g200_400.chr_regions.%A_%a.o
...
SAMPLE=Andmar.REF_AndHatt.sort.marked_dups.new.bam
## Andhae.REF_AndHatt.sort.marked_dups.new.bam
## Andmar.REF_AndHatt.sort.marked_dups.new.bam
...
## ./AndHat/iyAndHatt_8785v1_2.md_chr.fa.10kbp.regions.61.fb
...
freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    ...
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_AndMar_REF_AndHatt/"$BAM2VCF_NAME_200""$contig_regions_order".qual_20.g.vcf

freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    --ploidy 80 --pooled-discrete --genotype-qualities --use-best-n-alleles 4 \
    --bam $BAM_DIR/$SAMPLE -g 400 --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_AndMar_REF_AndHatt/"$BAM2VCF_NAME_400""$contig_regions_order".qual_20.g.vcf
