...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomHyp.10kb_g200_400.chr_regions.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomHyp.10kb_g200_400.chr_regions.%A_%a.o
#SBATCH --job-name=3_fb_variant_calling_4_bee_pools.BomPas_REF_BomHyp.10kb_g200_400.chr_regions
...
cd $VCF_OUT_DIR
mkdir fb_per_contig_BomPas_REF_BomHyp
...
SAMPLE=Bompas.REF_BomHypn.sort.marked_dups.new.bam
...
freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    ...
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomHyp/"$BAM2VCF_NAME_200""$contig_regions_order".qual_20.g.vcf

freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    --ploidy 68 --pooled-discrete --genotype-qualities --use-best-n-alleles 4 \
    --bam $BAM_DIR/$SAMPLE -g 400 --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_BomPas_REF_BomHyp/"$BAM2VCF_NAME_400""$contig_regions_order".qual_20.g.vcf
