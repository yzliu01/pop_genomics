...
#SBATCH --error=3_fb_variant_calling_4_bee_pools.AndHae_REF_AndHae.10kb_g600.chr_regions_No_best_4_allele.%A_%a.e
#SBATCH --output=3_fb_variant_calling_4_bee_pools.AndHae_REF_AndHae.10kb_g600.chr_regions_No_best_4_allele.%A_%a.o
#SBATCH --job-name=3_fb_variant_calling_4_bee_pools.AndHae_REF_AndHae.No_best_N_allele
...
SAMPLE=Andhae.REF_AndHae.sort.marked_dups.new.bam
## Andhae.REF_AndHae.sort.marked_dups.new.bam
...
freebayes-parallel $contig_regions 20 --fasta-reference $REF \
    --ploidy 78 --pooled-discrete --genotype-qualities \
    --bam $BAM_DIR/$SAMPLE -g 600 --strict-vcf --gvcf | \
    vcffilter -f "QUAL > 20" > $VCF_OUT_DIR/fb_per_contig_AndHae_REF_AndHae/"$BAM2VCF_NAME""$contig_regions_order".qual_20_no_best_4_alleles.g.vcf
