...
#SBATCH --error=popoolation_pi.New_REF_AndHae_marked_dups.%A_%a.e.txt
#SBATCH --output=popoolation_pi.New_REF_AndHae_marked_dups.%A_%a.o.txt
#SBATCH --job-name=popoolation_pi.New_REF_AndHae_marked_dups
...
IN_BAM=$(ls *New_REF_AndHae.sort.marked_dups.bam | sed -n ${SLURM_ARRAY_TASK_ID}p)
