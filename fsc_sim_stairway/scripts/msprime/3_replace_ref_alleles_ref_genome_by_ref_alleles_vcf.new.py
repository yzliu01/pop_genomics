...
def replace_ref_alleles(ref_fasta, vcf_file, output_fasta):
    ...
    for chrom_id, seq_list in ref_dict.items():
        seq_str = ''.join(seq_list).upper()  # Ensure uppercase sequence
        
        # Validate sequence
        if not seq_str or any(c not in "ACGTN" for c in seq_str):
            print(f"Warning: Chromosome {chrom_id} has an invalid or empty sequence. Skipping.")
            continue
        
        # Create SeqRecord with a valid sequence
        records.append(SeqIO.SeqRecord(seq=Seq(seq_str), id=chrom_id, description=''))
