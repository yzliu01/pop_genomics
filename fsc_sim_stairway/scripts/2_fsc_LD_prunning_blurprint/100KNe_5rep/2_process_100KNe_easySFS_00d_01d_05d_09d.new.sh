...
#sfs_100000Ne_"$sample_size"hapS_5rep_00d_01d_05d_09d_final.txt

for sample_size in {20,80,200};do
    if [ $sample_size == 20 ];then
        cat sfs_100000Ne_"$sample_size"hapS_5rep_00d_01d_05d_09d_final.txt |\
        cut -d "/" -f 1,3,5 | \
        sed -e 's/\.\///g' -e 's/_MAFpop0\.obs//' -e 's/MAFpop0\.obs//1' | \
        awk -F "\t" -v OFS="\t" '{$13=++a[$11]}1' | \
        awk '{ for (i = 1; i <= 11; ++i) printf $i"\t"; print "" $12$13}' > $out_sub_dir/sfs_Ne100000_hapS_20_80_200_5rep_00d_01d_05d_09d_final.blueprint.txt
        ...
        awk '{ for (i = 1; i <= 41; ++i) printf $i"\t"; print "" $42$43}' >> $out_sub_dir/sfs_Ne100000_hapS_20_80_200_5rep_00d_01d_05d_09d_final.blueprint.txt
    else
        cat sfs_100000Ne_"$sample_size"hapS_5rep_00d_01d_05d_09d_final.txt |\
        cut -d "/" -f 1,3,5 | \
        sed -e 's/\.\///g' -e 's/_MAFpop0\.obs//' -e 's/MAFpop0\.obs//1' | \
        awk -F "\t" -v OFS="\t" '{$103=++a[$101]}1' | \
        awk '{ for (i = 1; i <= 101; ++i) printf $i"\t"; print "" $102$103}' >> $out_sub_dir/sfs_Ne100000_hapS_20_80_200_5rep_00d_01d_05d_09d_final.blueprint.txt
    fi
done
## example
