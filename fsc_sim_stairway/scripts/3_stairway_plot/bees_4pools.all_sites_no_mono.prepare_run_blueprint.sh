...
#SBATCH --error=bee_pools_all_site_no_mono_sw.%A_%a.e.txt
#SBATCH --output=bee_pools_all_site_no_mono_sw.%A_%a.o.txt
...
blueprint_file=`ls $blueprint_file_dir/*sfs_600x.blueprint | sort -V`
...
run_blueprint_plot_sh=$(ls $blueprint_file_dir/*sfs_600x.blueprint.sh | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p)
