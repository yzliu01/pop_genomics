...
#SBATCH --error=Se_Bom12Bal_14Lap_10Mon_Ind.all_sites_no_singleton.new.%A_%a.e.txt
#SBATCH --output=Se_Bom12Bal_14Lap_10Mon_Ind.all_sites_no_singleton.new.%A_%a.o.txt
#SBATCH --job-name=Se_Bom12Bal_14Lap_10Mon_Ind.all_sites_no_singleton.new
...
blueprint_file=`ls $blueprint_file_dir/Se_*sfs_3*.blueprint | sort -V`
...
cd /home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2
run_blueprint_plot_sh=$(ls $blueprint_file_dir/Se_*sfs_3*.blueprint.sh | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p)
