...
#SBATCH --error=Se_Bompas_7Ind.all_sites_no_singleton.%A_%a.e.txt
#SBATCH --output=Se_Bompas_7Ind.all_sites_no_singleton.%A_%a.o.txt
#SBATCH --job-name=Se_Bompas_7Ind.all_sites_no_singleton
...
blueprint_file_dir=./stairway_plot_blueprint/Se_BomPas_7Ind
...
blueprint_file=`ls $blueprint_file_dir/Se_ind*.blueprint | sort -V`
...
run_blueprint_plot_sh=$(ls $blueprint_file_dir/Se_ind*.blueprint.sh | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p)
