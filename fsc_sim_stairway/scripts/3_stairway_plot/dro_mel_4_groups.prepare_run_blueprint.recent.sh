...
#SBATCH --error=dro_mel_4_groups_sw_recent.%A_%a.e.txt
#SBATCH --output=dro_mel_4_groups_sw_recent.%A_%a.o.txt
#SBATCH --job-name=dro_mel_4_groups_sw_recent
...
blueprint_file=`ls $blueprint_file_dir/dro_mel*.recent.blueprint | sort -V`
...
run_blueprint_plot_sh=$(ls $blueprint_file_dir/dro_mel*.recent.blueprint.sh | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p)
