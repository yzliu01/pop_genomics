...
pdf(paste0("StairwayPlot_AndMar.Median_75CI_95CI.pdf"),width = 25, height = 20)
## set number of plot in a figure
#par(mfcol=c(5,4),mar=c(4,4.4,3,1)+0.2,cex.lab=1.6, cex.axis=1.3,lwd=2)

par(mfcol=c(3,4),mar=c(4.0,4.5,5.8,1)+0.6,cex.lab=2.2, cex.axis=1.8,lwd=2.5)
layout(matrix(c(1:12),nrow=3,ncol=4,byrow=F),width=c(1,1),height=c(1,1))
...
for (group in c(
    ...
    if (group %in% c("AndMar_New_REF_AndMar.no_singleton_sfs_240_1500x_sm_genic", 
                    "AndMar_New_REF_AndMar.no_singleton_sfs_400_1500x_sm_genic"
                    )) {
        plot(1,1, type="n", xlim=c(10,5e5), ylim=c(1e3,1e5),log="x", xlab="", ylab="Effective population size",yaxt='n')
        ## ,xaxt='n',yaxt='n'
    ...
    } else if (group %in% c("AndMar_New_REF_AndMar.no_singleton_sfs_560_1500x_sm_genic"
                            )) {
        plot(1,1, type="n", xlim=c(10,5e5), ylim=c(1e3,1e5),log="x",xlab="Years ago", ylab="Effective population size",yaxt='n')
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_median,col=adjustcolor("red", alpha = 1),lwd=3)  # plot trajectory
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_2.5.,col=adjustcolor("blue", alpha = 0.8),lty=5,lwd=2.5)  # plot lower CI boundary
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_97.5.,col=adjustcolor("blue", alpha = 0.8),lty=5,lwd=2.5)  # plot upper CI boundary
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_12.5.,col=adjustcolor("goldenrod", alpha = 0.8),lty=5,lwd=2.5)  # plot lower CI boundary
        ...
        y_label <- c(1e3,1e4,2e4,4e4,6e4,8e4,1e5)
        axis(side=2, at=y_label,label=c(formatC(y_label,format="e",digits=0)))
    } else if (group %in% c("AndMar_New_REF_AndHat.no_singleton_sfs_240_1500x_sm_genic",
                            "AndMar_New_REF_AndHat.no_singleton_sfs_400_1500x_sm_genic",
                            "AndMar_New_REF_AndHae.no_singleton_sfs_240_1500x_sm_genic",
                            "AndMar_New_REF_AndHae.no_singleton_sfs_400_1500x_sm_genic",
                            "AndMar_New_REF_BomPas.no_singleton_sfs_240_1500x_sm_genic",
        ...
        plot(1,1, type="n", xlim=c(10,5e5), ylim=c(1e3,1e5),log="x",xlab="", ylab="",yaxt='n')
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_median,col=adjustcolor("red", alpha = 1),lwd=3)  # plot trajectory
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_2.5.,col=adjustcolor("blue", alpha = 0.8),lty=5,lwd=2.5)  # plot lower CI boundary
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_97.5.,col=adjustcolor("blue", alpha = 0.8),lty=5,lwd=2.5)  # plot upper CI boundary
        lines(get(paste0(group,"_Dat"))$year,get(paste0(group,"_Dat"))$Ne_12.5.,col=adjustcolor("goldenrod", alpha = 0.8),lty=5,lwd=2.5)  # plot lower CI boundary
        ...
        plot(1,1, type="n", xlim=c(10,5e5), ylim=c(1e3,1e5),log="x",xlab="Years ago",ylab="",yaxt='n')
    ...
    if (all(sapply(c("AndMar_New_REF_AndMar"), grepl, group))) {
