...
for (i in seq_along(events)) {
  ...
  if (is.null(x_limits)) x_limits <- c(1, 1000)

  # Get y-limits per event
  y_limits <- y_limits_list[[event]]
  if (is.null(y_limits)) y_limits <- c(1, 2000)
...
p <- ggplot() +
    ...
    #scale_x_continuous(trans = "log10",limits = x_limits, labels = label_number()) +
    ...
    labs(title = paste0("50000Ne_with_singleton_40S_", event),
         x = "Generations ago",
    ...
    theme(
      ...
      legend.position = if (i == 0) c(0.8, 0.7) else "none",
      #legend.position = if (i %in% c(1,2)) c(0.8, 0.6) else "none",
...
# Combine plots
combined_plot_100rep_no_log_with_singleton <- plot_list[[events[1]]] | plot_list[[events[2]]]
print(combined_plot_100rep_no_log_with_singleton)

# Save to PDF
ggsave(filename = "./combined_plot_40S_50G_05d_1000G_09d_100_replicates_with_95CI_with_singleton.no_log.pdf",
       plot = combined_plot_100rep_no_log_with_singleton, width = 12, height = 6)
...
library(patchwork)
pdf_file <- file.path(result_path,"combined_plot_40S_50G_05d_1000G_09d_100_replicates_with_95CI_with_singleton.log_no_log.pdf")
...
# another way
all_plots <- list(
combined_plot_100rep_log_with_singleton,
combined_plot_100rep_no_log_with_singleton
)
...
combined_plot <- wrap_plots(valid_plots, ncol = 1)
...
# Save to PDF
ggsave(pdf_file, combined_plot, width = 6, height = 6 * ceiling(length(valid_plots) / 2), limitsize = FALSE)
