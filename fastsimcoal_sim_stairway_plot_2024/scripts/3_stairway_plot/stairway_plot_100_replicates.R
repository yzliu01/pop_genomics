...
for (event in c(
    ...
    "1000G_09d"
        ...
        plot_name <- paste0("50000Ne_",event,"_20Chr_15Mb")
        print(plot_name)
...
for (rep in c(1:2)){
    ...
    assign(paste0("a",rep), read.table(mixedsort(sort(fs::dir_ls(path=paste0("./ft_sim_100000Ne_80hapS_1E_",event,"_20Chr_15Mb_",rep), recurse = 1, fail=TRUE, type = "file", glob = "*80hapS*.final.summary"))),header=T,sep="\t"))
    ...
    #                    get(paste0("a",rep))$Ne_2.5., get(paste0("a",rep))$Ne_97.5.,
    #                    get(paste0("a",rep))$Ne_12.5., get(paste0("a",rep))$Ne_87.5.)
    

    df <- get(paste0("a", rep))

    ## better
    data <- data.frame(df[["year"]],
                   df[["Ne_median"]],
                   df[["Ne_2.5."]],
                   df[["Ne_97.5."]],
                   df[["Ne_12.5."]],
...
file_path_hae <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[1]), recurse = 2, fail=TRUE, type = "file", glob = "*10x.P_10.final.summary")
...
file_path_mar <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[2]), recurse = 2, fail=TRUE, type = "file", glob = "*10x.P_10.final.summary")
...
file_path_pas <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[3]), recurse = 2, fail=TRUE, type = "file", glob = "*10x.P_10.final.summary")
...
file_path_vet <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[4]), recurse = 2, fail=TRUE, type = "file", glob = "*10x.P_10.final.summary")
...
# Save the plot
ggsave("non_log_plot_all_10x.no_singleton.tick_xy_log.pdf", p010x_final, width = 10, height = 8)
