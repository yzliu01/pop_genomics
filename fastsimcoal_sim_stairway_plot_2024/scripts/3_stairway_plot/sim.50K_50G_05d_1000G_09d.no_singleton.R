...
file_list_a <- c(             
                ...
                "AndHae_REF_AndHae.DP_10x.P_10.with_singleton",
                "AndMar_REF_AndMar.DP_10x.P_10.with_singleton",
                "BomPas_REF_BomPas.DP_10x.P_10.with_singleton",
                "BomVet_REF_BomVet.DP_10x.P_10.with_singleton"

)

file_list_b <- c(
                # hae
                expression(paste("Closest Ref: ", italic("A. haemorrhoa"), " | Divergence Time: 0 Mya | Cov: 10x")), 
                # mar
                expression(paste("Closest Ref: ", italic("A. marginata"), " | Divergence Time: 0 Mya | Cov: 10x")), 
                # pas
                expression(paste("Closest Ref: ", italic("B. pascuorum"), " | Divergence Time: 0 Mya | Cov: 10x")),          
                # vet
                expression(paste("Closest Ref: ", italic("B. veteranus"), " | Divergence Time: 0 Mya | Cov: 10x"))
)
...
file_path_hae <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[1]), recurse = 2, fail=TRUE, type = "file", glob = "*P_10.with_singleton.final.summary")
...
file_path_mar <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[2]), recurse = 2, fail=TRUE, type = "file", glob = "*P_10.with_singleton.final.summary")
...
file_path_pas <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[3]), recurse = 2, fail=TRUE, type = "file", glob = "*P_10.with_singleton.final.summary")
...
file_path_vet <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[4]), recurse = 2, fail=TRUE, type = "file", glob = "*P_10.with_singleton.final.summary")
...
p1_initial <- ggplot(merged_data_new, aes(x = year, y = Ne_median, group = species, color = species)) +
  ...
  labs(#title = "Median Ne Over Years by Species (10x)",
        title = "10x",
  ...
  scale_y_continuous(limits = c(1, 300000), labels = label_number(big.mark = "")) +
  scale_x_continuous(limits = c(10, 180000), labels = label_number(big.mark = "")) + # Set x-axis limits between 1 and 100,000
  ## add comma
  #scale_y_continuous(limits = c(1, 300000), labels = label_number(big.mark = ",")) + # Adding commas as thousand separators without scientific notation
  #scale_x_continuous(limits = c(10, 180000), labels = label_number(big.mark = ",")) + # Set x-axis limits between 1 and 100,000

  ## frame to select area on the plot
  annotate(geom = "rect", xmin = 10, xmax = 25000, ymin = 1000, ymax = 300000, 
...
#ggsave("/home/yzliu/eDNA/faststorage/yzliu/DK_proj/sofwtare/stairway_plot_v2/stairway_plot_v2.1.2/bee_pools_plot_new/plot_in_one_10x.new_non_log.pdf",width=6,height=6)
...
## combine:
p10x_final <- p1_initial + 
    ## position of zoom plot
    annotation_custom(ggplotGrob(p1_zoom), xmin = 50000, xmax = 180000, ymin = 150000, ymax = 300000) +
    ## position of annotation frame
    geom_rect(aes(xmin = 50000, xmax = 180000, ymin = 150000, ymax = 300000),
...
# Save the plot
ggsave("plot_all_10x.with_singleton.tick_x_log.new.pdf", p10x_final, width = 10, height = 8)
...
pdf_file <- file.path(result_path,"all_combined_3_5_7x.with_singleton.pdf")
...
pdf_file <- file.path(result_path,"all_combined_1_3_5_7_10x.with_and_no_singleton.tick_xy_log.pdf")
combined_plot <- p01x_final + p03x_final + p05x_final + p07x_final + p010x_final + 
                p1x_final + p3x_final + p5x_final + p7x_final + p10x_final + 
                plot_layout(ncol = 5)
ggsave(pdf_file,combined_plot,width = 30, height = 12, limitsize = FALSE)
