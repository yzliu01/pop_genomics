...
pdf(paste0("combined.10000Ne_200hapS_5rep_cons_1_1i_1_5i_1_9i_non_pruned.rotated_x_lab_final.new6.pdf"),width = 12, height = 12)
...
for (generation in c("50G","100G","500G","1000G")){
    ...
    for (event in c("cons","1.1i","1.5i","1.9i")){
        ## solo axis name plot
        #plot(1,1, type="n", xlim=c(1,5e5), ylim=c(5000,2.0e4),log="x", xlab="Generations ago", ylab="Effective population size",xaxt='n',yaxt='n')
...
#}
        ...
        if (plot_name %in% c("10000Ne_200hapS_1E_50G_cons_20Chr_15Mb", "10000Ne_200hapS_1E_100G_cons_20Chr_15Mb", "10000Ne_200hapS_1E_500G_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e5), ylim=c(5000,2.0e4),log="x", xlab="", ylab="Effective population size",xaxt='n',yaxt='n')
            #log="xy",xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            ...
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(5000,2.0e4),log="x", xlab="Generations ago", ylab="Effective population size",xaxt='n',yaxt='n')
            ...
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(5000,2.0e4),log="x", xlab="Generations ago", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(5000,10000,20000),label=c(format(c(5000,10000,20000),scientific=FALSE)))

        } else {
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(5000,2.0e4),log="x", xlab="", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(5000,10000,20000),label=c(format(c(5000,10000,20000),scientific=FALSE)))
        }
