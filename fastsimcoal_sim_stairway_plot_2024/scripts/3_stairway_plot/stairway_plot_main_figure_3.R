...
#pdf(paste0("stairway_plot_main_figure_instant_exponential_3.grid.test_list.pdf"),width = 12, height = 12)
...
#plot_index <- 1      # index counter

plot_list_runC <- list()
i <- 1

## 50kNe_40S
#plot_1 <- for (event in c("1.1i","1.5i","1.9i")){
for (event in c("1.5i","05i","2e_i_cons","2e_d_cons")){
    ## old naming format
    #for (event in c("00d","01d","05d","09d","05i")){
    ## modified naming format
    for (generation in c("100G")){
    #for (generation in c("50G","100G","500G","1000G")){
        ...
        plot_name <- paste0("50000Ne_40S_1E_",generation,"_",event)
        #print(plot_name)
        ## print names with newline 
        cat("\n",plot_name,"\n\n")
        
        if (plot_name %in% c(
            "50000Ne_40S_1E_100G_1.5i"
            )){
            plot(1,1, type="n", xlim=c(1,5e5), ylim=c(25000,1.0e5),log="x", xlab="Generations ago", ylab="Effective population size",xaxt='n',yaxt='n')
            #log="xy",xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            ...
            "50000Ne_40S_1E_100G_05i"
            )){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(25000,1.0e5),log="x", xlab="Generations ago", ylab="Effective population size",xaxt='n',yaxt='n')
            ...
            "50000Ne_40S_1E_100G_2e_i_cons"
            )){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,2.0e5),log="x", xlab="Generations ago", ylab="Effective population size",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            ...
            lablist.x <- as.vector(c(50,100,500,1000,10000,100000))
            axis(side=1,at=c(50,100,500,1000,10000,100000),labels=FALSE)
            text(x=c(50,100,500,1000,10000,100000),labels = c(format(lablist.x,scientific=FALSE)),par("usr")[3] - 0.3 * diff(y_lim),srt = 45,pos = 3, xpd = TRUE)
