...
for (model in demographic_models) {
  for (singleton in singleton_types) {
    ...
    for (event_size in event_sizes) {
      ...
      if (!is.null(plot_list[[key]])) {
        row_plots[[event_size]] <- plot_list[[key]]
      } else {
        row_plots[[event_size]] <- ggplot() + theme_void()
      }
    }
    row <- wrap_plots(row_plots, nrow = 1)
    plot_matrix[[paste0(model, singleton)]] <- row
  }
}

final_plot <- wrap_plots(plot_matrix, ncol = 1)
print(final_plot)


ggsave("500KNe_cons_1.1i.5i_1.9i_20S.pdf", final_plot, width = 10, height = 18) # 8 rows
