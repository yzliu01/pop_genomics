...
file_list_a <- c(             
                ...
                #5X
                "AndHae_REF_AndHae.DP_5x.P_10",
                "AndMar_REF_AndMar.DP_5x.P_10",
                "BomPas_REF_BomPas.DP_5x.P_10",
                "BomVet_REF_BomVet.DP_5x.P_10"
...
file_path_hae <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[1]), recurse = 2, fail=TRUE, type = "file", glob = "*5x.P_10.final.summary")
...
file_path_mar <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[2]), recurse = 2, fail=TRUE, type = "file", glob = "*5x.P_10.final.summary")
...
file_path_pas <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[3]), recurse = 2, fail=TRUE, type = "file", glob = "*5x.P_10.final.summary")
...
file_path_vet <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[4]), recurse = 2, fail=TRUE, type = "file", glob = "*5x.P_10.final.summary")
...
# Save the plot
ggsave("non_log_plot_all_5x.no_singleton.tick_xy_log.pdf", p05x_final, width = 10, height = 8)
...
#for (group in file_list){
    ...
    ## plot a frame
    file_path <- fs::dir_ls(path=paste0(result_path,"/",file_list_a[i]), recurse = 2, fail=TRUE, type = "file", glob = "*5x.P_10.final.summary")
