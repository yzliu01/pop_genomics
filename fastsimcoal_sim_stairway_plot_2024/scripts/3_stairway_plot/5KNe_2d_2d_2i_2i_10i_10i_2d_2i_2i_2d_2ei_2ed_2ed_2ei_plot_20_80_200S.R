...
## 50G
      ...
      if (xx <= 50) 5000*(0.9862^xx) else 5000*(0.9862^50)
    } else if (grepl("50G_10e_d_cons", plot_name)) {
      if (xx <= 50) 5000*(0.9549^xx) else 5000*(0.9549^50)
    } else if (grepl("50G_2e_i_cons", plot_name)) {
      if (xx <= 50) 5000*(1.0140^xx) else 5000*(1.0140^50)
    } else if (grepl("50G_10e_i_cons", plot_name)) {
      if (xx <= 50) 5000*(1.0472^xx) else 5000*(1.0472^50)
## 100G
    } else if (grepl("100G_2e_d_cons", plot_name)) {
      if (xx <= 100) 5000*(0.9931^xx) else 5000*(0.9931^100)
    } else if (grepl("100G_10e_d_cons", plot_name)) {
      if (xx <= 100) 5000*(0.9773^xx) else 5000*(0.9773^100)
    } else if (grepl("100G_2e_i_cons", plot_name)) {
      if (xx <= 100) 5000*(1.0069^xx) else 5000*(1.0069^100)
    } else if (grepl("100G_10e_i_cons", plot_name)) {
      if (xx <= 100) 5000*(1.0233^xx) else 5000*(1.0233^100)
## 500G
    } else if (grepl("500G_2e_d_cons", plot_name)) {
      if (xx <= 500) 5000*(0.9986^xx) else 5000*(0.9986^500)
    } else if (grepl("500G_10e_d_cons", plot_name)) {
      if (xx <= 500) 5000*(0.9954^xx) else 5000*(0.9954^500)
    } else if (grepl("500G_2e_i_cons", plot_name)) {
      if (xx <= 500) 5000*(1.0014^xx) else 5000*(1.0014^500)
    } else if (grepl("500G_10e_i_cons", plot_name)) {
      if (xx <= 500) 5000*(1.0046^xx) else 5000*(1.0046^500)
## 1000G
    } else if (grepl("1000G_2e_d_cons", plot_name)) {
      if (xx <= 1000) 5000*(0.9993^xx) else 5000*(0.9993^1000)
    } else if (grepl("1000G_10e_d_cons", plot_name)) {
      if (xx <= 1000) 5000*(0.9977^xx) else 5000*(0.9977^1000)
    } else if (grepl("1000G_2e_i_cons", plot_name)) {
      if (xx <= 1000) 5000*(1.0007^xx) else 5000*(1.0007^1000)
    } else if (grepl("1000G_10e_i_cons", plot_name)) {
      if (xx <= 1000) 5000*(1.0023^xx) else 5000*(1.0023^1000)

## two pop size change event at 500_100G
    } else if (grepl("100_500G_2i_2i", plot_name)) {
      if (xx <= 100) 5000 else if (100 < xx & xx <= 500) 2*5000 else 2*2*5000
    } else if (grepl("100_500G_2d_2i", plot_name)) {
      if (xx <= 100) 5000 else if (100 < xx & xx <= 500) 1/2*5000 else 1/2*2*5000
    } else if (grepl("100_500G_2i_2d", plot_name)) {
      if (xx <= 100) 5000 else if (100 < xx & xx <= 500) 2*5000 else 2*1/2*5000
    } else if (grepl("100_500G_2d_2d", plot_name)) {
      if (xx <= 100) 5000 else if (100 < xx & xx <= 500) 1/2*5000 else 1/2*1/2*5000
    } else if (grepl("100_500G_10d_10d", plot_name)) {
      if (xx <= 100) 5000 else if (100 < xx & xx <= 500) 1/10*5000 else 1/10*1/10*5000
    } else if (grepl("100_500G_2e_i_2e_d", plot_name)) {
      if (xx <= 100) 5000*(exp(log(2)/100*xx)) else if (xx <= 500) 5000*(exp(log(2)/100*100))*(exp(log(1/2)/400*(xx-100))) else 5000*(exp(log(2)/100*100))*(exp(log(1/2)/400*400))
    } else if (grepl("100_500G_2e_d_2e_i", plot_name)) {
      if (xx <= 100) 5000*(exp(log(1/2)/100*xx)) else if (xx <= 500) 5000*(exp(log(1/2)/100*100))*(exp(log(2)/400*(xx-100))) else 5000*(exp(log(1/2)/100*100))*(exp(log(2)/400*400))
## else
    } else {
      if (xx <= 100) 5000 else 5000 * 0
    }
  })
}
...
y_limits_list <- list(
  "100_500G_2i_2i"   = c(1, 25000),
  "100_500G_2d_2d"   = c(1, 15000),
  "100_500G_10d_10d"   = c(1, 15000),
  "100_500G_2d_2i"   = c(1, 15000),
  "100_500G_2i_2d"   = c(1, 15000),
  "100_500G_2e_i_2e_d"   = c(1, 15000),
  "100_500G_2e_d_2e_i"   = c(1, 15000)
)
...
#for (sample_size in c(20)) {
  for (singleton in c("_", "_no_singleton_")) {
    for (event in events) {
      ...
      for (rep in 1:n_reps) {
        rep_path <- paste0("./ft_sim_10000Ne", singleton, sample_size, "hapS_2E_", event, "_20Chr_15Mb_", rep)
...
# worked
for (sample_size in c(20, 80, 200)) {
  # First process all models with "_"
  row_with <- list()
  # Then process all models with "_no_singleton_"
  row_without <- list()
  
  for (model in demographic_models) {
    # With singleton
    key_with <- paste0(sample_size, "_", "100_500G_", model)
    row_with[[model]] <- if (!is.null(plot_list[[key_with]])) plot_list[[key_with]] else ggplot() + theme_void()
    
    # Without singleton
    key_without <- paste0(sample_size, "_no_singleton_", "100_500G_", model)
    row_without[[model]] <- if (!is.null(plot_list[[key_without]])) plot_list[[key_without]] else ggplot() + theme_void()
  }
  
  # Add both rows to the plot_rows list
  plot_rows[[paste0(sample_size, "_with")]] <- wrap_plots(row_with, nrow = 1)
  plot_rows[[paste0(sample_size, "_without")]] <- wrap_plots(row_without, nrow = 1)
}
...
# Save the plot
#ggsave("5KNe_plot_final.pdf", final_plot, width = 17.5, height = 13.5, limitsize = FALSE)
ggsave("5KNe_2d_2d_2i_2i_10i_10i_2d_2i_2i_2d_2ei_2ed_2ed_2ei_plot_20_80_200S.new1.pdf", final_plot, width = 17.5, height = 13.5, limitsize = FALSE) # 7 * 3 * 2 rows

# alternative way (for multiple events)
plot_rows <- list()
for (sample_size in c(20, 80, 200)) {
...
# Combine all rows vertically (with singleton first, then no_singleton for each sample size)
final_plot <- wrap_plots(plot_rows, ncol = 1)
