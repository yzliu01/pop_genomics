...
for (i in seq_along(events)) {
  ...
  for (rep in 1:n_reps) {
    rep_path <- paste0("./ft_sim_100000Ne_no_singleton_80hapS_1E_", event, "_20Chr_15Mb_", rep)
  ...
  p <- ggplot() +
    ...
    scale_y_continuous(trans = "log10", limits = c(1000, 200000), labels = label_number()) +

    labs(title = paste0("50000Ne_no_singleton_", event),
         x = "Generations ago",
...
# Combine plots
combined_plot_95CI_no_singleton <- plot_list[[events[1]]] | plot_list[[events[2]]]
print(combined_plot_95CI_no_singleton)

# Save to PDF
ggsave(filename = "./combined_plot_40S_50G_05d_1000G_09d_5_replicates_with_95CI_no_singleton.new_log_xy.pdf",
       plot = combined_plot_95CI_no_singleton, width = 12, height = 6)
...
pdf_file <- file.path(result_path,"combined_plot_40S_50G_05d_1000G_09d_5_replicates_with_95_75CI_with_and_no_singleton.new.pdf")
combined_plot <- (
                combined_plot_95CI_with_singleton + combined_plot_75CI_with_singleton +
                combined_plot_95CI_no_singleton + combined_plot_75CI_no_singleton
                ) + plot_layout(ncol = 4, nrow = 2)
ggsave(pdf_file,combined_plot,width = 12, height = 24, limitsize = FALSE)


library(patchwork)
pdf_file <- file.path(result_path,"combined_plot_40S_50G_05d_1000G_09d_5_replicates_with_95_75CI_with_and_no_singleton.new.pdf")
#final_plot <- wrap_plots(all_plots, nrow = 4)
#ggsave(pdf_file, final_plot, width = 10, height = 20)


# another way
all_plots <- list(
  combined_plot_95CI_with_singleton,
  combined_plot_75CI_with_singleton,
  combined_plot_95CI_no_singleton,
  combined_plot_75CI_no_singleton
)
...
# Save to PDF
ggsave(pdf_file, combined_plot, width = 12, height = 3 * ceiling(length(valid_plots) / 2), limitsize = FALSE)
