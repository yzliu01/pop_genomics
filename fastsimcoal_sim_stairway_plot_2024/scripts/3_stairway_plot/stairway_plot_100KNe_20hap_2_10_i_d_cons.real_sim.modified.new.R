...
pdf(paste0("combined.50000Ne_10S_5rep_2_10_i_d_cons_non_pruned.pdf"),width = 12, height = 12)
#pdf(paste0("combined.50000Ne_10S_5rep_non_pruned.real_sim.rotated_x_lab-.pdf"),width = 20, height = 25)
...
for (generation in c("50G","100G","500G","1000G")){
    ...
    for (event in c("2e_d_cons","10e_d_cons","2e_i_cons","10e_i_cons")){
        ...
        plot_name <- paste0("50000Ne_10S_1E_",generation,"_",event,"_20Chr_15Mb")
        #print(plot_name)
        ## print names with newline 
        cat("\n",plot_name,"\n\n")
        
        if (plot_name %in% c("50000Ne_10S_1E_50G_2e_d_cons_20Chr_15Mb", "50000Ne_10S_1E_100G_2e_d_cons_20Chr_15Mb", "50000Ne_10S_1E_500G_2e_d_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e5), ylim=c(1,2e5),log="x", cex.axis= 0.8, xlab="", ylab="Effective population size",xaxt='n',yaxt='n')
            #log="xy",xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(25000,50000,200000),label=c(format(c(25000,50000,200000),scientific=FALSE)))

        } else if (plot_name %in% c("50000Ne_10S_1E_1000G_2e_d_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,2e5),log="x", cex.axis= 0.8, xlab="Generations ago", ylab="Effective population size",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(25000,50000,200000),label=c(format(c(25000,50000,200000),scientific=FALSE)))

        } else if (plot_name %in% c("50000Ne_10S_1E_1000G_2e_i_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,2e5),log="x", cex.axis= 0.8, xlab="Generations ago", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(50000,100000,200000),label=c(format(c(50000,100000,200000),scientific=FALSE)))

        } else if (plot_name %in% c("50000Ne_10S_1E_50G_2e_i_cons_20Chr_15Mb","50000Ne_10S_1E_100G_2e_i_cons_20Chr_15Mb","50000Ne_10S_1E_500G_2e_i_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,2e5),log="x", cex.axis= 0.8, xlab="", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(50000,100000,200000),label=c(format(c(50000,100000,200000),scientific=FALSE)))

        } else if (plot_name %in% c("50000Ne_10S_1E_50G_10e_i_cons_20Chr_15Mb","50000Ne_10S_1E_100G_10e_i_cons_20Chr_15Mb","50000Ne_10S_1E_500G_10e_i_cons_20Chr_15Mb")){
            
            ## high Ne (attention to duplicate)

            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,5e5),log="x", cex.axis= 0.8, xlab="", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(50000,300000,500000),label=c(format(c(50000,300000,500000),scientific=FALSE)))

        } else if (plot_name %in% c("50000Ne_10S_1E_1000G_10e_i_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,5e5),log="x", cex.axis= 0.8, xlab="Generations ago", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(50000,300000,500000),label=c(format(c(50000,300000,500000),scientific=FALSE)))

        } else if (plot_name %in% c("50000Ne_10S_1E_1000G_10e_d_cons_20Chr_15Mb")){
            plot(1,1, type="n", xlim=c(1,5e6), ylim=c(1,2e5),log="x", cex.axis= 0.8, xlab="Generations ago", ylab="",xaxt='n',yaxt='n')
            #xlab="Generations ago", ylab="Effective population size"
            title(line=2)
            axis(side=2, at=c(5000,50000,200000),label=c(format(c(5000,50000,200000),scientific=FALSE)))
        ...
        #axis(side=2, at=c(25000,50000,200000,75000,100000,125000,150000,180000,200000,300000),label=c(format(c(25000,50000,200000,75000,100000,125000,150000,180000,200000,300000),scientific=FALSE)))
        ...
        ##50000Ne_10S_1E_50G_10e_i_cons_20Chr_15Mb
        ## distance to x axis
        if (plot_name %in% c("50000Ne_10S_1E_50G_10e_i_cons_20Chr_15Mb","50000Ne_10S_1E_100G_10e_i_cons_20Chr_15Mb","50000Ne_10S_1E_500G_10e_i_cons_20Chr_15Mb","50000Ne_10S_1E_1000G_10e_i_cons_20Chr_15Mb")){
            text(x=c(50,100,500,1000,10000,100000),labels = c(format(lablist.x,scientific=FALSE)),par("usr")[3] - 80000,srt = 45,pos = 3, xpd = TRUE)
        } else if (plot_name %in% c("50000Ne_10S_1E_50G_2e_i_cons_20Chr_15Mb","50000Ne_10S_1E_100G_2e_i_cons_20Chr_15Mb","50000Ne_10S_1E_500G_2e_i_cons_20Chr_15Mb","50000Ne_10S_1E_1000G_2e_i_cons_20Chr_15Mb")) {
            text(x=c(50,100,500,1000,10000,100000),labels = c(format(lablist.x,scientific=FALSE)),par("usr")[3] - 30000,srt = 45,pos = 3, xpd = TRUE)
        } else {
            text(x=c(50,100,500,1000,10000,100000),labels = c(format(lablist.x,scientific=FALSE)),par("usr")[3] - 30000,srt = 45,pos = 3, xpd = TRUE)
        }

        ## add plot title in each plot
        mtext(side=3,text=paste0("50000Ne_10S_1E_",generation,"_",event),cex=0.8,line=1.2)
        ...
        for (rep in c(1:5)){
            ...
            assign(paste0("a",rep), read.table(mixedsort(sort(fs::dir_ls(path=paste0("./ft_sim_100000Ne_20hapS_1E_",generation,"_",event,"_20Chr_15Mb_",rep), recurse = 1, fail=TRUE, type = "file", glob = "*20hapS*.final.summary"))),header=T,sep="\t"))
            print(head(get(paste0("a",rep)),n=2L))
            #paste0("data_ft_sim_50000Ne_10S_1E_",generation,"_",event,"_20Chr_15Mb")
...
#}
        ...
        x <- 0:100000
        #plot_name <- c("50000Ne_10S_1E_1000G_1.1i_20Chr_15Mb","50000Ne_10S_1E_1000G_1.5i_20Chr_15Mb","50000Ne_10S_1E_1000G_1.9i_20Chr_15Mb","50000Ne_10S_1E_1000G_0.5e_d_1.5i_20Chr_15Mb")
        ## category
        ...
        fx <- function(x) {
            ...
            ## gen_100
                ...
                ## piewise 
                if (x <= 50) {
                    return(50000*(0.9862^x))
                } else {
                    return(50000*(0.9862^50))
                }
            } else if (all(sapply(c("50G","G_2e_i_cons"), grepl, plot_name))) {
                if (x <= 50) {
                    return(50000*(1.0140^x))
                } else {
                    return(50000*(1.0140^50))
                }
            } else if (all(sapply(c("50G","G_10e_d_cons"), grepl, plot_name))) {
                if (x <= 50) {
                    return(50000*(0.9549^x))
                } else {
                    return(50000*(0.9549^50))
                }
            } else if (all(sapply(c("50G","G_10e_i_cons"), grepl, plot_name))) {
                if (x <= 50) {
                    return(50000*(1.0472^x))
                } else {
                    return(50000*(1.0472^50))
                }
...
#axis(1,at=c(50,100,500,1000,10000,100000),labels=FALSE)
lablist.x <- as.vector(c(50,100,500,1000,10000,100000))
text(x=c(50,100,500,1000,10000,100000),labels = format(lablist.x,scientific=TRUE),par("usr")[3] - 405,srt = 30,pos = 3, xpd = TRUE)
dev.off()
