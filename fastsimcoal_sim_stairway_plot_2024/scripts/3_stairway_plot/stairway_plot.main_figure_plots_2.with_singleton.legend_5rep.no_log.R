...
x_limits_list <- list(
  ...
  "1000G_1.5i"   = c(1, 1500),
  "1000G_1.9i" = c(1, 1500),
"ft_sim_10000Ne_20hapS_1E_100G_1.5i" = c(1, 500),
"ft_sim_100000Ne_20hapS_1E_100G_1.5i" = c(1, 750),
"ft_sim_1000000Ne_20hapS_1E_100G_1.5i" = c(1, 750),
"ft_sim_10000Ne_80hapS_1E_100G_1.5i" = c(1, 500),
"ft_sim_100000Ne_80hapS_1E_100G_1.5i" = c(1, 750),
"ft_sim_1000000Ne_80hapS_1E_100G_1.5i" = c(1, 750),
"ft_sim_10000Ne_200hapS_1E_100G_1.5i" = c(1, 500),
"ft_sim_100000Ne_200hapS_1E_100G_1.5i" = c(1, 750),
"ft_sim_1000000Ne_200hapS_1E_100G_1.5i" = c(1, 750)
...
for (i in seq_along(events)) {
  ...
  if (is.null(y_limits)) y_limits <- c(10, 200000)
...
p <- ggplot() +
    ...
    scale_y_continuous(limits = y_limits, labels = label_number()) +
    ...
    theme(
      ...
      legend.position = if (i == 1) c(0.7, 0.72) else "none",
...
combined_plot_95CI_with_singleton <- grid.arrange(
...
# Save to PDF
ggsave(filename = "./combined_plot_10_40_100S_100G_05d_5_replicates_with_75CI_with_singleton.no_log.new.pdf",
       plot = combined_plot_95CI_with_singleton, width = 12, height = 8)
...
combined_plot_95CI_with_singleton <- wrap_plots(plot_list_new, nrow = 3,ncol=4, brow = FALSE)
print(combined_plot_95CI_with_singleton)

combined_plot_95CI_with_singleton <- wrap_plots(plot_list, nrow = 3,ncol=4, brow = FALSE)
print(combined_plot_95CI_with_singleton)
...
# Save to PDF
ggsave(filename = "./combined_plot_10_40_100S_100G_05d_5_replicates_with_75CI_with_singleton.no_log.pdf",
       plot = combined_plot_95CI_with_singleton, width = 12, height = 8)
...
for (i in seq_along(events)) {
  ...
  # Create model data (independent of replicates)
  model_years <- seq(min(long_df$year), max(long_df$year), length.out = 100)
  model_df <- data.frame(
    year = model_years,
    Ne_true = fx(model_years, event)
  )
  ...
  # Create base plot
  p <- ggplot() +
    # First plot the model (independent of replicates)
    geom_step(data = model_df, 
              aes(x = year, y = Ne_true, color = "Model"), 
              size = 1.2, alpha = 0.9) +
    
    # Then plot the replicates
    geom_ribbon(data = long_df, 
                aes(x = year, ymin = Ne_12.5., ymax = Ne_87.5., group = replicate, fill = "75% CI"),
                alpha = 0.3) +
    geom_line(data = long_df, 
...
# Combine plots with empty spaces
empty_plot <- ggplot() + theme_void()

combined_plot <- grid.arrange(
  plot_list, plot_list, plot_list, empty_plot,
  plot_list, plot_list, plot_list, empty_plot,
  plot_list, plot_list, plot_list, empty_plot,
  nrow = 3,
  ncol = 4
...
# Save to PDF
ggsave(
  filename = "./combined_plot_10_40_100S_100G_05d_5_replicates_with_75CI_with_singleton.no_log.new.pdf",
  plot = combined_plot, 
  width = 12, 
  height = 8
)
