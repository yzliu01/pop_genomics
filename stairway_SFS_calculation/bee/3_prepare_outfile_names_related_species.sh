...
for species in {Hae_39,Mar_40,Pas_34,Vet_29};do
    if [ "$species" = "Hae_39" ];then
        cat "$species"_dipS.DP_3_5_7x.sfs |\
        # cut: the delimiter must be a single character
        #cut -d "/" -f 1,3,5 | \
        awk -F "./" '{print $1 $2 $3}' |\
        sed -e 's/concated\.//g' -e 's/_New//g' -e 's/100kb_g1500x_regions.GQ_issue_solved.SNP_softmask_genic_bi_FMT_//g' -e 's/_noMS.shuf//g' -e 's/.equal_self.sfs//g' -e 's/_1500x//g' > ./"$species"_dipS.DP_3_5_7x.sfs.blueprint.txt
        # AndHae_REF_AndHae.DP_10x.P_05
        ...
        #awk '{ for (i = 1; i <= 40; ++i) printf $i"\t"; print "" $41$42}' > $out_sub_dir/"$species"_dipS.DP_3_5_7x.sfs.blueprint.txt
        ...
        cat "$species"_dipS.DP_3_5_7x.sfs |\
        awk -F "./" '{print $1 $2 $3}' |\
        sed -e 's/concated\.//g' -e 's/_New//g' -e 's/100kb_g1500x_regions.all_chr.sorted.GQ_issue_solved.SNP_softmask_genic_bi_FMT_//g' -e 's/_noMS.shuf//g' -e 's/.equal_self.sfs//g' -e 's/_1500x//g' > ./"$species"_dipS.DP_3_5_7x.sfs.blueprint.txt
        ...
        cat "$species"_dipS.DP_3_5_7x.sfs |\
        awk -F "./" '{print $1 $2 $3}' |\
        sed -e 's/concated\.//g' -e 's/_New//g' -e 's/100kb_g1500x_regions.SNP_softmask_genic_bi_FMT_//g' -e 's/_noMS.shuf//g' -e 's/.equal_self.sfs//g' -e 's/_1500x//g' > ./"$species"_dipS.DP_3_5_7x.sfs.blueprint.txt
...
## replace patterns in file names
#ls *DP_3_5_7x.sfs.blueprint.txt
#Hae_39_dipS.DP_3_5_7x.sfs.blueprint.txt
#Mar_40_dipS.DP_3_5_7x.sfs.blueprint.txt
#Pas_34_dipS.DP_3_5_7x.sfs.blueprint.txt
#Vet_29_dipS.DP_3_5_7x.sfs.blueprint.txt

## from: BomPas_REF_BomSyl.100kb_g1500x_regions.all_chr.sorted.SNP_softmask_genic_bi_FMT_DP340_noMS
## to: BomPas_REF_BomHyp.DP340
for species in {Hae_39,Mar_40,Pas_34,Vet_29};do

## edit in file
sed -i -e 's/100kb_g1500x_regions.//g' -e 's/all_chr.//g' -e 's/sorted.//g' -e 's/SNP_softmask_genic_bi_FMT_//g' -e 's/_noMS//g' -e 's/GQ_issue_solved.//g' -e 's/alt_//g' -e 's/cp\.//g' -e 's/cp_new_1.//g' ${species}_dipS.DP_3_5_7x.sfs.blueprint.txt
