...
#SBATCH --mem 50g
##SBATCH --array=1-402%402
#SBATCH --array=1-2%2
##SBATCH --time=00:10:00
#SBATCH --time=00:40:00
#SBATCH --error=angsd_likelihood_sfs_chr_hae.%A_%a.e
#SBATCH --output=angsd_likelihood_sfs_chr_hae.%A_%a.o
#SBATCH --job-name=angsd_likelihood_sfs_chr_hae
...
ref=Andrena_haemorrhoa-GCA_910592295.1-softmasked.fa
ref_chr=$(cut -f 1 "$REF_DIR/$ref".fai | sed -n ${SLURM_ARRAY_TASK_ID}p)
...
ref_bed=$REF_MASKED_DIR/Andrena_haemorrhoa-GCA_910592295.1-softmasked_ref_gene.conca_sorted.bed
md_bed=$REF_MASKED_DIR/Andrena_haemorrhoa-GCA_910592295.1-softmasked_ref_gene.conca_sorted.md.bed
awk '{print $1 ":" $2+1 "-" $3}' $ref_bed > $md_bed
...
bam=Andhae.New_REF_AndHae.sort.marked_dups.bam
...
## -r file:regions.txt
##angsd -bam <(echo $bam) -rf $REF_MASKED_DIR/$ref_bed -doSaf 1 -anc $REF_DIR/$ref -out "$SAF_OUT_DIR/$sfs_out".bed -GL 2 -P 20 -doMajorMinor 1 -minMapQ 1 -minQ 20
#angsd -bam <(echo $bam) -r $ref_chr -doSaf 1 -anc $REF_DIR/$ref -out "$SAF_OUT_DIR/$sfs_out.$ref_chr" -GL 2 -P 40 -doMajorMinor 1 -minMapQ 1 -minQ 20
angsd -bam <(echo $bam) -r $ref_chr -doSaf 1 -anc $REF_DIR/$ref -out "$SAF_OUT_DIR/$sfs_out.$ref_chr" -GL 2 -P 40 \
    -doCounts 1 -setMinDepth 40 -setMaxDepth 200 -doMajorMinor 1 -minMapQ 30 -minQ 20
...
#realSFS -r $md_bed "$SAF_OUT_DIR/$sfs_out.$ref_chr".saf.idx -maxIter 100 -P 20 > "$SAF_OUT_DIR/$sfs_out.$ref_chr".sfs
idx_dir=/home/yzliu/eDNA/faststorage/yzliu/DK_proj/data/bee_proj_data/SFS_data/angsd
#realSFS -r 1:6-6446 $idx_dir/Andhae.1.saf.idx -maxIter 100 -P 20 > $SAF_OUT_DIR/Andhae.1.sfs
realSFS $idx_dir/Andhae.1.saf.idx -maxIter 100 -P 20 > $SAF_OUT_DIR/Andhae.1.sfs
# 57429624.764998 793119.421156 20116.813850 

#in R
setwd("/home/yzliu/eDNA/faststorage/yzliu/DK_proj/data/bee_proj_data/SFS_data/angsd--")
sfs<-scan("Andhae.1.sfs")
barplot(sfs[-1])
